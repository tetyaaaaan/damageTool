...
/**
 * Materialize the authoritative behaviorSpec shard 07 source records for the
 * 6.7 -> 7.0 transition.
 *
 ...
 * canonical data.  Entity 10000063 is the overlap with shard 06 and is
...
const sourceNamespace = "genshin-db-behavior-shard07";
...
const outputPath = path.join(transitionRoot, "behavior-shard-07-snapshot.json");
const checkpointPath = path.join(transitionRoot, "behavior-shard-07-capture-checkpoint.json");
...
const shardId = "behavior:behaviorSpec:semanticDecisionRequired:standard:07";
const overlapEntityIds = ["10000063"];
...
const resolvedEntities = [
    { entityId: "10000064", name: "Yun Jin", nameJa: "雲菫", providerId: 6401, slug: "yunjin", slugCandidates: ["yunjin"] },
    { entityId: "10000065", name: "Kuki Shinobu", nameJa: "久岐忍", providerId: 6501, slug: "kukishinobu", slugCandidates: ["kukishinobu"] },
    { entityId: "10000066", name: "Kamisato Ayato", nameJa: "神里綾人", providerId: 6601, slug: "kamisatoayato", slugCandidates: ["kamisatoayato"] },
    { entityId: "10000067", name: "Collei", nameJa: "コレイ", providerId: 6701, slug: "collei", slugCandidates: ["collei"] },
    { entityId: "10000068", name: "Dori", nameJa: "ドリー", providerId: 6801, slug: "dori", slugCandidates: ["dori"] },
    { entityId: "10000069", name: "Tighnari", nameJa: "ティナリ", providerId: 6901, slug: "tighnari", slugCandidates: ["tighnari"] },
    { entityId: "10000070", name: "Nilou", nameJa: "ニィロウ", providerId: 7001, slug: "nilou", slugCandidates: ["nilou"] }
];
...
async function fetchBytes(url) {
    const response = await fetch(url, { headers: { "User-Agent": "damageTool-genshin-behavior-shard-07" } });
    if (!response.ok) {
...
function buildSnapshot() {
    ...
    const claim = {
        ...
        shard: {
            ...
            overlapPolicy: "alreadyCapturedByBehaviorShard06",
            overlapEvidence: [{
                entityId: "10000063",
                previousShardId: "behavior:behaviorSpec:semanticDecisionRequired:standard:06",
        ...
        travelerVariantPolicy: {
            ...
            note: "No Traveler entity is in shard07; any future Traveler queue entry must remain unresolved until its variant identity is explicit."
        },
        coverage: {
...
function validateBehaviorShard07Snapshot(snapshot, { repositoryRoot: root = repositoryRoot } = {}) {
    const reasons = [];
    ...
    if (shard?.shardId !== shardId || shard?.authoritativeCandidateCount !== queue.candidateIds.length
        ...
        || shard?.overlapPolicy !== "alreadyCapturedByBehaviorShard06") reasons.push("authoritativeShardBindingInvalid");
    ...
    if (!Array.isArray(shard?.overlapEvidence) || shard.overlapEvidence.length !== 1
        || shard.overlapEvidence[0]?.entityId !== "10000063"
        || shard.overlapEvidence[0]?.previousShardId !== "behavior:behaviorSpec:semanticDecisionRequired:standard:06"
...
function validateCheckpoint(checkpoint, { repositoryRoot: root = repositoryRoot } = {}) {
    ...
    if (!materializedPath || !fs.existsSync(materializedPath)) reasons.push("materializedSnapshotMissing");
        ...
        if (snapshot && (checkpoint.materializedSnapshot.fieldDigest !== snapshot.fieldDigest
            || !validateBehaviorShard07Snapshot(snapshot, { repositoryRoot: root }).valid)) reasons.push("materializedSnapshotInvalid");
    }
    for (const side of ["before", "after"]) {
...
module.exports = {
    ...
    shardId,
    sourceNamespace,
    validateBehaviorShard07Snapshot,
    validateCheckpoint,
    writeSnapshot
};
