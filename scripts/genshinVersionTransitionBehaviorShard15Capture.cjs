...
/**
 * Capture the authoritative behaviorSpec shard 15 for the 6.7 -> 7.0
 ...
 * mapper currently has no entries for 10000124-31;
...
const sourceNamespace = "genshin-db-behavior-shard15";
...
const outputPath = path.join(transitionRoot, "behavior-shard-15-snapshot.json");
const checkpointPath = path.join(transitionRoot, "behavior-shard-15-capture-checkpoint.json");
...
const shardId = "behavior:behaviorSpec:semanticDecisionRequired:standard:15";
const travelerEntityIds = ["10000005", "10000007"];
const expectedCandidateCount = 100;
const expectedCandidateIdsDigest = "2db0350a66c2db499f3d856abea3d1828c7c68156fe7e87ba71de830c85bfafe";
const expectedEntityIdsDigest = "1be70e13a9b10c363e028e6edcf126e7c6071463cbb7a0b28d3a335ac2cd0e83";
const expectedEntityIds = [
    "10000124", "10000125", "10000126", "10000127",
    "10000128", "10000129", "10000130", "10000131"
];
...
const resolvedEntities = [
    { entityId: "10000124", name: "Jahoda", nameJa: "ヤフォダ", slug: "jahoda", slugCandidates: ["jahoda"], providerId: 12401 },
    { entityId: "10000125", name: "Columbina", nameJa: "コロンビーナ", slug: "columbina", slugCandidates: ["columbina"], providerId: 12501 },
    { entityId: "10000126", name: "Zibai", nameJa: "兹白", slug: "zibai", slugCandidates: ["zibai"], providerId: 12601 },
    { entityId: "10000127", name: "Illuga", nameJa: "イルーガ", slug: "illuga", slugCandidates: ["illuga"], providerId: 12701 },
    { entityId: "10000128", name: "Varka", nameJa: "ファルカ", slug: "varka", slugCandidates: ["varka"], providerId: 12801 },
    { entityId: "10000129", name: "Lohen", nameJa: "ロエン", slug: "lohen", slugCandidates: ["lohen"], providerId: 12901 },
    { entityId: "10000130", name: "Linnea", nameJa: "リンネア", slug: "linnea", slugCandidates: ["linnea"], providerId: 13001 },
    { entityId: "10000131", name: "Nicole", nameJa: "ニコル", slug: "nicole", slugCandidates: ["nicole"], providerId: 13101 }
];
...
function queueShard() {
    ...
    if (candidateIds.length !== expectedCandidateCount || new Set(candidateIds).size !== candidateIds.length || !candidateIds.every((id) => /^behavior:100\d{5}:[^:]+:.+$/.test(id))) {
        throw new Error(`authoritative behaviorSpec shard 15 candidate inventory invalid: ${candidateIds.length}`);
    }
    ...
    const entityIdsDigest = digestStable(entityIds);
    if (candidateIdsDigest !== expectedCandidateIdsDigest) throw new Error(`shard 15 candidate digest mismatch: expected ${expectedCandidateIdsDigest}, got ${candidateIdsDigest}`);
    if (entityIdsDigest !== expectedEntityIdsDigest || entityIdsDigest !== digestStable(expectedEntityIds)) throw new Error(`shard 15 entity inventory mismatch: ${entityIds.join(",")}`);
...
async function fetchBytes(url) {
    const response = await fetch(url, { headers: { "User-Agent": "damageTool-genshin-behavior-shard-15" } });
    if (!response.ok) {
...
function buildSnapshot(root = repositoryRoot) {
    ...
    const claim = {
        ...
        travelerVariantPolicy: {
            ...
            note: "No Traveler entity is in shard15; any future Traveler queue entry must remain unresolved until its variant identity is explicit."
        },
        coverage: {
...
function validateBehaviorShard15Snapshot(snapshot, { repositoryRoot: root = repositoryRoot } = {}) {
    const reasons = [];
...
function validateCheckpoint(checkpoint, { repositoryRoot: root = repositoryRoot } = {}) {
    ...
    if (!materializedPath || !fs.existsSync(materializedPath)) reasons.push("materializedSnapshotMissing");
        ...
        if (snapshot && (checkpoint.materializedSnapshot.fieldDigest !== snapshot.fieldDigest || !validateBehaviorShard15Snapshot(snapshot, { repositoryRoot: root }).valid)) reasons.push("materializedSnapshotInvalid");
    }
...
module.exports = {
    ...
    shardId,
    sourceNamespace,
    validateBehaviorShard15Snapshot,
    validateCheckpoint,
    writeSnapshot
};
