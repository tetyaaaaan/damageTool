...
/**
 * Candidate-scoped r2 audit for BehaviorSpec shard05.
 *
...
const SNAPSHOT_PATH = path.join(ROOT, "games", "genshin", "data", "v2", "version-transitions", "6.7-to-7.0", "behavior-shard-05-snapshot.json");
const SOURCE_AUDIT_PATH = path.join(ROOT, "games", "genshin", "data", "v2", "reviews", "r2-behavior-spec-shard05-source-audit.json");
const SOURCE_AUDIT_REPORT_PATH = path.join(ROOT, "reports", "genshin-r2-behavior-spec-shard05-source-audit.md");
const ARTIFACT_PATH = path.join(ROOT, "games", "genshin", "data", "v2", "reviews", "r2-behavior-spec-shard05-g06-deferral.json");
const REPORT_PATH = path.join(ROOT, "reports", "genshin-r2-behavior-spec-shard05-g06-deferral.md");
const SCHEMA_PATH = path.join(ROOT, "games", "genshin", "data", "schema", "r2-behavior-spec-shard05-g06-deferral.schema.json");
...
const SHARD_ID = "behavior:behaviorSpec:semanticDecisionRequired:standard:05";
const FRONTIER_ID = "genshin-7.0-behavior-spec-shard05-persisted-provider-frontier";
...
const PRIOR_SOURCE_AUDITS = [1, 2, 3, 4].map((n) => path.join(ROOT, "games", "genshin", "data", "v2", "reviews", `r2-behavior-spec-shard0${n}-source-audit.json`));
...
function buildSourceAudit() {
    ...
    const output = {
        schemaVersion: 1,
        kind: "genshinR2BehaviorSpecShard05SourceAudit",
        status: "passed",
        generatedAt: GENERATED_AT,
        generator: { name: path.basename(__filename), version: "genshinR2BehaviorSpecShard05SourceAudit/1" },
        scope: {
            transitionId: TRANSITION_ID,
            targetGameVersion: TARGET_GAME_VERSION,
            layer: "behaviorSpec",
            shardId: SHARD_ID,
            selection: "next 100 sorted ready BehaviorSpec tasks after the registered shard01 through shard04, with resolved identity, verified transition raw, and complete local claim schema",
...
function decisionFor(candidate, refs, frontier, projectedTask) {
    ...
    return {
        id: `r2-behavior-shard05-g06-deferral:${candidate.candidateId}`,
...
function buildAudit() {
    ...
    const output = {
        schemaVersion: 1,
        kind: "genshinR2BehaviorSpecShard05G06Deferral",
        policyId: POLICY_ID,
        transitionId: TRANSITION_ID,
        fromGameVersion: "6.7",
        targetGameVersion: TARGET_GAME_VERSION,
...
function validateSourceAudit(audit, { compareCurrent = false } = {}) {
    ...
    if (!audit || audit.kind !== "genshinR2BehaviorSpecShard05SourceAudit") return { valid: false, reasons: ["artifactMissing"] };
...
function validateAudit(audit, { compareCurrent = false } = {}) {
    const reasons = [];
    const add = (reason) => { if (!reasons.includes(reason)) reasons.push(reason); };
    if (!audit || audit.kind !== "genshinR2BehaviorSpecShard05G06Deferral") return { valid: false, reasons: ["artifactMissing"] };
