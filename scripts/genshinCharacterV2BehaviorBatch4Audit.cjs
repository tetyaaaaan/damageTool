...
/** Audit the deterministic behavior-v2 character batch 4 artifact. */
...
const {
    ...
    defaultOutputRoot,
    stableJson
} = require("./genshinCharacterV2BehaviorBatch4Generate.cjs");

const repositoryRoot = path.resolve(__dirname, "..");
const defaultReportJson = path.join(repositoryRoot, "reports", "genshin-character-v2-behavior-batch-4.json");
const defaultReportMarkdown = path.join(repositoryRoot, "reports", "genshin-character-v2-behavior-batch-4.md");
const SAFE_MEASUREMENT_PATHS = new Set([
    ...
    "/execution/maxTriggers", "/execution/charges", "/execution/maxInstances", "/execution/offField",
    "/execution/snapshot", "/execution/area", "/execution/targetCount", "/energy/burstCost",
    "/energy/gain", "/energy/particleCount", "/energy/particleElement", "/elementApplication/element",
    "/elementApplication/gaugeUnits", "/elementApplication/icdInterval", "/elementApplication/icdHitRule",
    "/elementApplication/icdScope"
]);
...
function emptyGenerated() { return { generatedFromDisk: false, ...buildDataset({ dataRoot: defaultDataRoot }) }; }
...
function auditBehaviorBatch4({ dataRoot = defaultDataRoot, outputRoot = defaultOutputRoot } = {}) {
    ...
    if (generated.batch !== 4) errors.push("batch must be 4");
    if (stableJson(generated.batchIds || []) !== stableJson(BATCH_IDS)) errors.push("batch IDs do not match fixed batch 4 inventory");
    ...
    if (generated.generatedFromDisk) {
        ...
        if (!fs.existsSync(indexPath)) errors.push("index.json is missing");
            ...
            const expectedIndex = {
                schemaVersion: generated.schemaVersion, kind: "genshinBehaviorV2BatchIndex", generator: generated.generator,
                policy: generated.policy, summary: generated.summary, batch: generated.batch, batchIds: generated.batchIds,
                excludedPilotIds: generated.excludedPilotIds, generatedFrom: generated.generatedFrom, byCharacter: generated.byCharacter
            };
    ...
    Object.entries(generated.specs || {}).forEach(([id, spec]) => {
        ...
        if (!spec || spec.id !== id || !spec.entity?.kind || !spec.entity?.id || !spec.entity?.component
            ...
            || spec.interpretation.method !== "deterministicParser" || !spec.timing || !spec.execution || !spec.lifecycle || !spec.energy
            ...
            || spec.runtime?.status !== "blocked" || !Array.isArray(spec.runtime.modifierIds) || spec.runtime.modifierIds.length !== 0
            || spec.runtime.generator !== null || !Array.isArray(spec.unknownFields) || !spec.inventoryCandidateId
            || !generated.inventoryCandidates?.[spec.inventoryCandidateId] || !Array.isArray(spec.inventoryCandidateIds) || !Array.isArray(spec.unknownCandidateIds)) {
        ...
        Object.entries(spec.timing).concat(Object.entries(spec.execution), Object.entries(spec.energy), Object.entries(spec.elementApplication || {})).forEach(([field, value]) => {
            const section = Object.prototype.hasOwnProperty.call(spec.timing, field) ? "timing" : Object.prototype.hasOwnProperty.call(spec.execution, field) ? "execution" : Object.prototype.hasOwnProperty.call(spec.energy, field) ? "energy" : "elementApplication";
            const fieldPath = `/${section}/${field}`;
            if (!SAFE_MEASUREMENT_PATHS.has(fieldPath) || !isMeasurement(value)) schemaViolations.push(`${id}:${fieldPath}`);
        });
        if (!["none", "refresh", "extend", "replace", "independent", "unknown"].includes(spec.lifecycle.refreshMode)
            || !["duration", "triggerLimit", "consumed", "destroyed", "stateEnd", "unknown"].includes(spec.lifecycle.expiration)) schemaViolations.push(`${id}:lifecycle`);
        Object.values(spec.verification.claims).forEach((claim) => { if (claim.status !== "needsReview" || !Array.isArray(claim.sourceRefs)) malformedSpecs.push(`${id}:claim`); });
        ...
        explicitByPath.forEach((candidates, fieldPath) => {
            ...
            if (candidates.length === 1 && SAFE_MEASUREMENT_PATHS.has(fieldPath) && candidates[0].value !== null && ["set", "replace", "setMaximum"].includes(candidates[0].operation)) {
            ...
            } else if (candidates.length === 1 && SAFE_LIFECYCLE_PATHS.has(fieldPath) && candidates[0].value !== null && candidates[0].operation === "set") {
                ...
                if (!spec.verification.discrepancies.some((entry) => ["INVENTORY_NULL_RESET", "INVENTORY_OPERATION_REQUIRES_REVIEW"].includes(entry.code) && entry.fieldPath === fieldPath)) inventoryCandidateViolations.push({ owner: id, fieldPath, reason: "nullResetNotRecorded" });
            }
        });
        Object.entries(spec.timing).concat(Object.entries(spec.execution), Object.entries(spec.energy), Object.entries(spec.elementApplication || {})).forEach(([field, measurement]) => {
            const section = Object.prototype.hasOwnProperty.call(spec.timing, field) ? "timing" : Object.prototype.hasOwnProperty.call(spec.execution, field) ? "execution" : Object.prototype.hasOwnProperty.call(spec.energy, field) ? "energy" : "elementApplication";
            const fieldPath = `/${section}/${field}`;
    ...
    Object.entries(generated.byCharacter || {}).forEach(([characterId, index]) => {
        if (!index || index.characterId !== characterId || index.canonical !== 0 || !Array.isArray(index.sourceRecordIds)
            || !Array.isArray(index.inventoryCandidateIds) || !Array.isArray(index.specIds) || !Array.isArray(index.modifierIds) || index.modifierIds.length !== 0) {
    ...
    const summary = {
        ...
        contract: {
            malformedSources: malformedSources.length, malformedInventory: malformedInventory.length, malformedSpecs: malformedSpecs.length,
            malformedByCharacter: malformedByCharacter.length, missingSourceRefs: missingSourceRefs.length, missingInventoryRefs: missingInventoryRefs.length,
            inputDigestMismatches: inputDigestMismatches.length, unknownFieldViolations: unknownFieldViolations.length,
            numericInferenceViolations: numericInferenceViolations.length, inventoryCandidateViolations: inventoryCandidateViolations.length,
    ...
    const characterCoverage = BATCH_IDS.map((characterId) => {
        ...
        return {
            characterId, characterName: index.characterName || null, sourceRecords: index.sourceRecordIds?.length || 0,
            inventoryCandidates: index.inventoryCandidateIds?.length || 0, specs: index.specIds?.length || 0,
            ...
            modifiers: index.modifierIds?.length || 0, canonical: index.canonical ?? null
        };
    });
    return {
        schemaVersion: 2, status: summary.status, summary, errors, malformedSources, malformedInventory, malformedSpecs,
        malformedByCharacter, missingSourceRefs, missingInventoryRefs, inputDigestMismatches, unknownFieldViolations,
        numericInferenceViolations, inventoryCandidateViolations, schemaViolations, characterCoverage,
        generatedFromDisk: Boolean(generated.generatedFromDisk)
    };
}
...
function markdownReport(report) {
    ...
    const lines = [
        "# Genshin behavior v2 batch 4 audit", "", `- Status: **${summary.status}**`,
        `- Fixed batch characters: ${summary.batchCharacters}`, `- Raw source records: ${summary.sourceRecords}`,
        `- Batch-local inventory candidates: ${summary.inventoryCandidates}`, `- Source inventory candidates: ${summary.sourceInventoryCandidates}`,
        `- Explicit inventory candidates: ${summary.explicitInventoryCandidates}`, `- Unknown inventory candidates: ${summary.unknownInventoryCandidates}`,
...
if (require.main === module) {
    ...
    const report = auditBehaviorBatch4({ dataRoot, outputRoot });
    writeReports({
        report,
        reportJson: process.env.GENSHIN_CHARACTER_V2_BEHAVIOR_BATCH_4_REPORT_JSON || defaultReportJson,
        reportMarkdown: process.env.GENSHIN_CHARACTER_V2_BEHAVIOR_BATCH_4_REPORT_MD || defaultReportMarkdown
    });
...
module.exports = {
    auditBehaviorBatch4,
    auditGenshinCharacterV2BehaviorBatch4: auditBehaviorBatch4,
    defaultReportJson,
