...
/**
 * Candidate-scoped, evidence-only frontier for the next 100 weapon
 * candidates whose already-captured genshin-db refinement vectors are
 * numeric matches and whose current queue has no consumer lane.  This module
 * consumes persisted evidence only.  It does
...
const SHARD_ID = "weapons-primary-numeric-shard03";
const SHARD_SIZE = 100;
const SHARD_OFFSET = 200;
const FRONTIER_ID = "weapons-primary-numeric-shard03-independent-field-frontier-7.0";
const FRONTIER_POLICY_ID = "genshin-7.0-weapon-primary-numeric-shard-independent-field-frontier";
const EXPECTED_ARTIFACT_SHA256 = "e9412205156019f3d32d1189f5c5862b419f01bb16a6b885f0a71dc40fb62d3e";
const EXPECTED_FIELD_DIGEST = "5f81c2deefcfa845764caa056ccea5d3aa691f444931099dfcf511103b1301ca";
...
const OUTPUT_PATH = path.join(ROOT, "games", "genshin", "data", "v2", "reviews", "r2-weapon-primary-field-search-shard03.json");
const SCHEMA_PATH = path.join(ROOT, "games", "genshin", "data", "schema", "r2-weapon-primary-field-search-shard03.schema.json");
...
function loadInputs() {
    ...
    // Shard01 has already been registered in the mutable coordination queue,
    // so the comparison artifact's historical 381-task selector no longer
    // matches the current queue.  Raw comparison pins remain independently
    // valid; queue state is retained as an observation for this shard.
    ...
    if (consumerFreeNumericIds.length < SHARD_OFFSET + SHARD_SIZE) {
        throw new Error(`consumerFreeNumericCandidateCount:${consumerFreeNumericIds.length}`);
    }
    const candidateIds = consumerFreeNumericIds.slice(SHARD_OFFSET, SHARD_OFFSET + SHARD_SIZE);
    ...
    return {
        ...
        candidateIds, queueById, consumerFreeNumericIds, officialClaimsById, finiteLedger, policyEntries, providerAssessments
    };
}
    ...
    return [
        {
            ...
            candidateScopeStatus: `${officialCandidateIds.length}CandidateRawHits`,
...
function buildAudit() {
    ...
    const officialRawCandidateCount = candidateIds.filter((id) => (inputs.officialClaimsById.get(id) || []).length > 0).length;
    const officialClaimCount = candidateIds.reduce((total, id) => total + (inputs.officialClaimsById.get(id) || []).length, 0);
    const frontier = {
        id: FRONTIER_ID,
        ...
        candidateIds,
        searchScope: "Exact numeric-match weapon candidates ranked 201-300 after excluding all numeric candidates whose current queue consumerStatus is notApplicable; persisted primary raw pins, official HoYoWiki capture, finite provider substitution assessments, finite 30-candidate weapon ledger, source catalog policy, and candidate-specific repository evidence only. No network fetch or raw refetch.",
        ...
        negativeResult: `No second independent provider family supplies a complete immutable target-7.0 candidate×claim field bundle for this shard. HoYoWiki has ${officialRawCandidateCount} mutable raw hits; the other ${candidateIds.length - officialRawCandidateCount} have no candidate-specific independent field record in the persisted bounded inputs. No simple majority or semantic inference is used.`,
        candidateRawHitSummary: {
            hoyowikiCandidates: officialRawCandidateCount,
            hoyowikiClaimFields: officialClaimCount,
            candidatesWithoutIndependentCandidateFieldRecord: candidateIds.length - officialRawCandidateCount,
    ...
    const artifact = {
        ...
        scope: {
            shardId: SHARD_ID,
            selection: "primary comparison records with status numericAgreementOnlyNotSemanticIdentity and current queue consumerStatus=notApplicable, localeCompare candidateId ascending, offset 200, next 100",
            candidateCount: candidateIds.length,
...
function decisionArtifactRef(audit) {
    ...
    if (ref.sha256 !== EXPECTED_ARTIFACT_SHA256) {
        throw new Error(`weaponShardArtifactDigestUnexpected:${ref.sha256}`);
    }
    if (audit.fieldDigest !== EXPECTED_FIELD_DIGEST) {
...
function decisionForWeaponTask(task, candidate, audit, artifactRef) {
    ...
    return {
        id: `r2-weapon-primary-shard03-deferral:${task.candidateId}`,
...
function isAllowedWeaponShard03ExternalEvidenceWait(task, audit) {
    const laneList = Array.isArray(task?.deferredLanes) ? task.deferredLanes : [];
    const frontier = task?.searchFrontier;
    const fieldComparison = task?.fieldComparison || {};
    if (laneList.length !== 0 || task?.layer !== "weaponEffectSpec" || task?.dataset !== "weapons"
        || task?.terminalState !== "blocked" || task?.primaryBlockReason !== "sourceMissing"
        || task?.task?.kind !== "sourceProvider" || task?.task?.status !== "reopenOnTrigger"
        || task?.task?.reason !== "sourceMissing" || task?.task?.autoProcessableNow === true
        || task?.autoProcessableNow === true || task?.consumerStatus !== "notApplicable"
        || task?.identityConsistency?.status === "ambiguous" || fieldComparison.hasMismatch === true) return false;
    if (frontier?.frontierId !== FRONTIER_ID || frontier?.policyId !== FRONTIER_POLICY_ID
        || frontier?.status !== "searchExhausted" || frontier?.exhausted !== true
        || frontier?.scopeMatched !== true || frontier?.candidateIdDigest !== audit?.scope?.candidateIdDigest
        || frontier?.auditDigest !== audit?.fieldDigest) return false;
    return Array.isArray(audit?.scope?.candidateIds)
        && audit.scope.candidateIds.includes(String(task.candidateId));
}
...
function prepareRegistry({ audit = null, compareCurrent = null, sourceRegistry = null } = {}) {
    ...
    for (const id of candidateIds) {
        ...
        if (!isAllowedWeaponShard03ExternalEvidenceWait(baseTask, audit)) {
    ...
    const registry = {
        ...
        policyId: work.POLICY,
        targetGameVersion: TARGET_GAME_VERSION,
        note: "In-memory exact weapon primary shard03 preview; not persisted.",
        decisions,
    ...
    const combinedExpectedKpi = {
        ...
        addedCandidateShards: { behaviorSpecShard01: 100, weaponPrimaryShard01: 100, weaponPrimaryShard03: 100 },
        expectedAfterBothShards: {
            disposed: (baseSummary.workDisposed || 0) + (frontierAlreadyRegistered ? 0 : SHARD_SIZE),
            evidenceDeferred: (baseSummary.evidenceDeferred || 0) + (frontierAlreadyRegistered ? 0 : SHARD_SIZE),
            pending: Math.max(0, (baseSummary.workPending || 0) - (frontierAlreadyRegistered ? 0 : SHARD_SIZE)),
            strictVerified: 0
        },
        baseMatchesExpected: Number.isInteger(baseSummary.workDisposed)
            && Number.isInteger(baseSummary.evidenceDeferred)
            && Number.isInteger(baseSummary.workPending)
    };
    return {
...
module.exports = {
    ...
    SHARD_OFFSET,
    REQUIRED_CLAIMS,
    isAllowedWeaponShard03ExternalEvidenceWait,
    SHARD_SIZE,
    stableJson,
    digest,
    buildAudit,
