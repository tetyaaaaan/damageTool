...
const {
    ...
    defaultOutputRoot,
    stableJson
} = require("../../scripts/genshinCharacterV2BehaviorBatch4Generate.cjs");
const { auditBehaviorBatch4 } = require("../../scripts/genshinCharacterV2BehaviorBatch4Audit.cjs");
...
test("batch 4 fixes the requested ten non-pilot character IDs", () => {
    ...
    assert.equal(artifact.summary.inventoryInputCandidates, 155);
    assert.equal(artifact.summary.explicitCandidateOperations, 103);
    assert.equal(artifact.summary.unknownCandidateIds, 52);
    assert.equal(artifact.summary.sourceInventoryCandidates, 155);
    assert.equal(artifact.summary.explicitInventoryCandidates, 103);
    assert.equal(artifact.summary.unknownInventoryCandidates, 52);
...
test("raw source pointers and behavior-inventory candidates resolve without inference", () => {
    ...
    assert.equal(explicitCount, 103);
    assert.equal(unknownCount, 52);
    assert.deepEqual(artifact.modifiers, {});
});

test("batch 4 generation, split files, and audit are deterministic", () => {
    ...
    const report = auditBehaviorBatch4({ dataRoot: defaultDataRoot, outputRoot: defaultOutputRoot });
