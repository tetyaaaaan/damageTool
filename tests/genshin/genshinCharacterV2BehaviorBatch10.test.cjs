...
const {
    ...
    defaultOutputRoot,
    stableJson
} = require("../../scripts/genshinCharacterV2BehaviorBatch10Generate.cjs");
const { auditBehaviorBatch10 } = require("../../scripts/genshinCharacterV2BehaviorBatch10Audit.cjs");
...
test("batch 10 fixes the requested ten non-pilot character IDs", () => {
    ...
    assert.equal(artifact.summary.inventoryInputCandidates, 172);
    assert.equal(artifact.summary.explicitCandidateOperations, 119);
    assert.equal(artifact.summary.unknownCandidateIds, 53);
    assert.equal(artifact.summary.sourceInventoryCandidates, 172);
    assert.equal(artifact.summary.explicitInventoryCandidates, 119);
    assert.equal(artifact.summary.unknownInventoryCandidates, 53);
...
test("raw source pointers and behavior-inventory candidates resolve without inference", () => {
    ...
    assert.equal(explicitCount, 119);
    assert.equal(unknownCount, 53);
    assert.deepEqual(artifact.modifiers, {});
});

test("batch 10 generation, split files, and audit are deterministic", () => {
    ...
    const report = auditBehaviorBatch10({ dataRoot: defaultDataRoot, outputRoot: defaultOutputRoot });
