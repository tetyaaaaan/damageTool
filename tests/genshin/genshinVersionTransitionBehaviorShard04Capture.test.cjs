...
test("shard 04 binds raw records and records the concrete Sucrose 7.0 change", () => {
    const snapshot = capture.buildSnapshot();
    const records = snapshot.claim.resolvedEntities.flatMap((entity) => entity.records);
    assert.equal(records.length, 16);
    assert.equal(records.filter((record) => record.comparison.status === "rawRecordMatch").length, 15);
...
test("shard 04 checked-in snapshot is deterministic and validates its raw files", () => {
...
test("shard 04 complete checkpoint is deterministic and remains fail closed", () => {
    const checkedIn = JSON.parse(fs.readFileSync(capture.checkpointPath, "utf8"));
    assert.deepEqual(checkedIn, capture.buildCheckpoint());
    const validation = capture.validateCheckpoint(checkedIn, { repositoryRoot });
    assert.equal(validation.valid, true, validation.reasons.join("; "));
    assert.equal(checkedIn.status, "complete");
    assert.equal(checkedIn.revisions.before.completedArtifactCount, 16);
    assert.equal(checkedIn.revisions.after.completedArtifactCount, 16);
    assert.equal(checkedIn.gateEligibility.canIssueEligibilityCertificate, false);
    assert.equal(checkedIn.gateEligibility.canPromoteCanonical, false);
});
...
test("shard 04 schema declares fixed queue, raw bindings, and fail-closed fields", () => {
