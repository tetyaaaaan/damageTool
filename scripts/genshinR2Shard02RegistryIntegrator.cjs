...
/**
 * Read-only integration preview for the already audited Shard02 artifacts.
 *
 * Shard01 is the established 393-disposition base.  This seam reuses the
 * existing Shard01 integrator for that base, then joins exactly one behavior
 * shard02 and one weapon shard02 frontier/decision set against a fresh
 * terminal queue.  It is intentionally not an evidence producer and does
 * not edit the authoritative queue, checkpoint, Runtime, canonical data, or
 * HSR files.  The optional write path is restricted to the two registries
...
const baseIntegrator = require("./genshinR2Shard01RegistryIntegrator.cjs");
const behaviorShard = require("./genshinR2BehaviorSpecShard02G06Deferral.cjs");
const weaponShard = require("./genshinR2WeaponPrimaryFieldSearchShard02.cjs");
...
const BEHAVIOR_ARTIFACT_SHA256 = "b874f5df56c52d1d0e715bdd88d51410105889b4d243847397ef969076752ace";
const BEHAVIOR_SOURCE_AUDIT_SHA256 = "a47e7a5cc31d2af55b221ae9c9f55707bb2d4925c3bb50a0ad8a4ab128040f03";
const WEAPON_ARTIFACT_SHA256 = "6ee94161af04ceafb0596e764db4d65839ebbf499e6af329211bd7f09f371d69";
...
function assertShardArtifactPins() {
    ...
    if (sha256(behaviorArtifactBytes) !== BEHAVIOR_ARTIFACT_SHA256) throw new Error("behaviorShard02ArtifactShaMismatch");
    if (sha256(behaviorSourceBytes) !== BEHAVIOR_SOURCE_AUDIT_SHA256) throw new Error("behaviorShard02SourceAuditShaMismatch");
    if (sha256(weaponArtifactBytes) !== WEAPON_ARTIFACT_SHA256) throw new Error("weaponShard02ArtifactShaMismatch");
    ...
    if (!behaviorValidation.valid) throw new Error(`behaviorShard02ArtifactInvalid:${behaviorValidation.reasons.join(",")}`);
    const behaviorSourceValidation = behaviorShard.validateSourceAudit(behaviorSource, { compareCurrent: false });
    if (!behaviorSourceValidation.valid) throw new Error(`behaviorShard02SourceAuditInvalid:${behaviorSourceValidation.reasons.join(",")}`);
    const weaponValidation = weaponShard.validateAudit(weaponAudit, { compareCurrent: false });
    const ignoredMutableInputReasons = /^(fileBytesMismatch|fileDigestMismatch):(reports\/genshin-evidence-task-queue\.json|games\/genshin\/data\/v2\/source-search-frontiers\.json|reports\/genshin-candidate-source-coverage-frontier-inventory\.json)$/;
    const weaponImmutableErrors = weaponValidation.reasons.filter((reason) => !ignoredMutableInputReasons.test(reason));
    if (weaponImmutableErrors.length > 0) throw new Error(`weaponShard02ArtifactInvalid:${weaponImmutableErrors.join(",")}`);
    if (behaviorAudit.fieldDigest !== "46a36045f69afe25ddb2f0b4408e0a50f976fc851d6110d2e7c93b2840b89b14") {
        throw new Error("behaviorShard02ArtifactFieldDigestMismatch");
    }
    if (behaviorSource.fieldDigest !== workDigest(behaviorSource)) {
        // `validateSourceAudit` already checks its producer digest.  Keep this
        // branch explicit so a missing/malformed field digest cannot pass a
        // SHA-only integration by accident.
        throw new Error("behaviorShard02SourceAuditFieldDigestMismatch");
    }
    return { behaviorAudit, behaviorSource, weaponAudit };
}

// Behavior shard02 uses the stable-json digest implementation from its
// producer, not the work registry's generic digest.  This helper is only used
// for the producer's already persisted source audit and is defined locally to
// avoid exposing another mutable integration API.
...
function validateBehaviorFrontier(audit, entry) {
    ...
    if (!entry || entry.id !== behaviorShard.FRONTIER_ID || entry.status !== "searchExhausted"
        ...
        throw new Error("behaviorShard02FrontierEntryMismatch");
    }
    if (entry.evidenceRefs.some((ref) => work.isMutableCoordinationArtifactPath(ref))) {
        throw new Error("behaviorShard02MutableFrontierEvidenceRef");
    }
}
...
function weaponFrontierEntry(audit, prepared) {
    ...
    if (!preview || preview.id !== expected.id || preview.policyId !== expected.policyId
        ...
        throw new Error("weaponShard02FrontierPreviewMismatch");
    }
    for (const task of prepared.tasks) {
        ...
        if (!frontier || frontier.frontierId !== expected.id
            ...
            throw new Error(`weaponShard02TaskFrontierMismatch:${task?.candidateId || "<missing>"}`);
        }
    }
    return {
        id: expected.id,
...
function buildIntegration() {
    ...
    // After the explicit write, validate the persisted exact state in place.
    // Do not remove and rebuild registered Shard02 frontiers merely to prove
    // idempotence; the current immutable artifacts and fresh terminal tasks
    // remain the authority for the same predicates and queue-task digests.
    if (registeredTargets.length === TARGET_FRONTIER_IDS.length) {
        if (currentSource.frontiers.length !== EXPECTED.finalFrontiers
            || currentWork.decisions?.length !== EXPECTED.finalDecisions) throw new Error("registeredShard02CountInvalid");
        ...
        if (behaviorTasks.some((task) => !task) || weaponTasks.some((task) => !task)) throw new Error("registeredShard02TaskMissing");
        if (behaviorTasks.some((task) => !work.isAllowedShard02ExternalEvidenceWait(task))) throw new Error("registeredShard02BehaviorPredicateInvalid");
        if (weaponTasks.some((task) => !work.isAllowedWeaponShard02ExternalEvidenceWait(task))) throw new Error("registeredShard02WeaponPredicateInvalid");
        ...
        if (behaviorDecisions.some((decision) => !decision) || weaponDecisions.some((decision) => !decision)) throw new Error("registeredShard02DecisionMissing");
        assertDecisionBindings(behaviorDecisions, tasksById, "registeredBehaviorShard02");
        assertDecisionBindings(weaponDecisions, tasksById, "registeredWeaponShard02");
    ...
    if (registeredTargets.length !== 0) throw new Error("partialShard02Registration");
    const base = baseIntegrator.buildIntegration();
    if (!base.valid) throw new Error(`shard01BaseInvalid:${base.errors.join(",")}`);
    if (base.sourceRegistry.frontiers.length !== EXPECTED.baseFrontiers
        || base.workRegistry.decisions.length !== EXPECTED.baseDecisions) throw new Error("shard01BaseCountInvalid");

    const { behaviorAudit, behaviorSource, weaponAudit } = pinned;
    const behaviorIds = behaviorAudit.scope.candidateIds.map(String);
    const weaponIds = weaponAudit.scope.candidateIds.map(String);
    if (behaviorIds.length !== EXPECTED.addedBehaviorCandidates || weaponIds.length !== EXPECTED.addedWeaponCandidates) throw new Error("shard02CandidateCountInvalid");
    const overlap = behaviorIds.filter((id) => weaponIds.includes(id));
    if (overlap.length > 0) throw new Error(`shard02CandidateOverlap:${overlap.join(",")}`);
    ...
    if (baseCollisions.length > 0) throw new Error(`shard02DecisionAlreadyRegistered:${[...new Set(baseCollisions)].join(",")}`);
    ...
    if (frontierCollisions.length > 0) throw new Error(`shard02FrontierAlreadyRegistered:${frontierCollisions.join(",")}`);

    const behaviorEntry = behaviorShard.frontierEntry(behaviorAudit.frontier, behaviorAudit.evidence.sourceRefs);
    validateBehaviorFrontier(behaviorAudit, behaviorEntry);
    const weaponPrepared = weaponShard.prepareRegistry({
        audit: weaponAudit,
        compareCurrent: false,
        sourceRegistry: base.sourceRegistry
    });
    if (!weaponPrepared.valid) throw new Error(`weaponShard02PreparationInvalid:${weaponPrepared.errors.join(",")}`);
    ...
    const behaviorTasks = behaviorIds.map((id) => tasksById.get(id));
    const weaponTasks = weaponIds.map((id) => tasksById.get(id));
    if (behaviorTasks.some((task) => !task) || weaponTasks.some((task) => !task)) throw new Error("shard02TaskBindingMissing");
    if (behaviorTasks.some((task) => !work.isAllowedShard02ExternalEvidenceWait(task))) throw new Error("shard02BehaviorTaskPredicateInvalid");
    if (weaponTasks.some((task) => !work.isAllowedWeaponShard02ExternalEvidenceWait(task))) throw new Error("shard02WeaponTaskPredicateInvalid");

    const behaviorDecisions = behaviorAudit.decisions.map(clone);
    const weaponDecisions = weaponPrepared.decisions.map(clone);
    assertDecisionBindings(behaviorDecisions, tasksById, "behaviorShard02");
    assertDecisionBindings(weaponDecisions, tasksById, "weaponShard02");
    ...
    return {
        ...
        kpi,
        shards: {
            behaviorShard01: clone(base.shards.behavior),
            weaponShard01: clone(base.shards.weapon),
            behaviorShard02: {
                candidateIds: behaviorIds,
                candidateIdDigest: behaviorAudit.scope.candidateIdDigest,
                artifactFieldDigest: behaviorAudit.fieldDigest,
                artifactSha256: BEHAVIOR_ARTIFACT_SHA256,
                sourceAuditDigest: behaviorSource.fieldDigest,
                decisions: behaviorDecisions.length
            },
            weaponShard02: {
                candidateIds: weaponIds,
