...
const SHARD02_SOURCE_PATH = path.join(ROOT, "games", "genshin", "data", "v2", "reviews", "r2-behavior-spec-shard02-source-audit.json");
const SHARD03_SOURCE_PATH = path.join(ROOT, "games", "genshin", "data", "v2", "reviews", "r2-behavior-spec-shard03-source-audit.json");
const SNAPSHOT_PATH = path.join(ROOT, "games", "genshin", "data", "v2", "version-transitions", "6.7-to-7.0", "behavior-shard-04-snapshot.json");
...
const SOURCE_AUDIT_PATH = path.join(ROOT, "games", "genshin", "data", "v2", "reviews", "r2-behavior-spec-shard04-source-audit.json");
const SOURCE_AUDIT_REPORT_PATH = path.join(ROOT, "reports", "genshin-r2-behavior-spec-shard04-source-audit.md");
const ARTIFACT_PATH = path.join(ROOT, "games", "genshin", "data", "v2", "reviews", "r2-behavior-spec-shard04-g06-deferral.json");
const REPORT_PATH = path.join(ROOT, "reports", "genshin-r2-behavior-spec-shard04-g06-deferral.md");
const SCHEMA_PATH = path.join(ROOT, "games", "genshin", "data", "schema", "r2-behavior-spec-shard04-g06-deferral.schema.json");

const GENERATED_AT = "2026-08-31T00:00:00.000Z";
const GENERATOR_VERSION = "genshinR2BehaviorSpecShard04G06Deferral/1";
const SOURCE_GENERATOR_VERSION = "genshinR2BehaviorSpecShard04SourceAudit/1";
const POLICY_ID = "genshin-goal-2026-08-28-r2";
const TRANSITION_ID = "genshin:6.7->7.0";
const TARGET_GAME_VERSION = "7.0";
const SHARD_ID = "behavior:behaviorSpec:semanticDecisionRequired:standard:04";
const FRONTIER_ID = "genshin-7.0-behavior-spec-shard04-persisted-provider-frontier";
...
function persistedShardIds() {
    const paths = [SHARD01_SOURCE_PATH, SHARD02_SOURCE_PATH, SHARD03_SOURCE_PATH];
    const ids = new Set();
    for (const file of paths) {
        if (!fs.existsSync(file)) continue;
        const artifact = readJson(file);
        for (const id of artifact.scope?.candidateIds || []) ids.add(String(id));
    }
    return ids;
}
...
function buildSourceAudit() {
    ...
    const output = {
        schemaVersion: 1,
        kind: "genshinR2BehaviorSpecShard04SourceAudit",
        status: "passed",
        ...
        scope: {
            ...
            layer: "behaviorSpec",
            shardId: SHARD_ID,
            selection: "next 100 sorted ready BehaviorSpec tasks after the registered shard01, shard02, and shard03, with resolved identity, verified transition raw, and complete local claim schema",
...
function decisionFor(candidate, refs, frontier, projectedTask = null) {
    ...
    return {
        id: `r2-behavior-shard04-g06-deferral:${candidate.candidateId}`,
...
function buildAudit() {
    ...
    const output = {
        schemaVersion: 1,
        kind: "genshinR2BehaviorSpecShard04G06Deferral",
        policyId: POLICY_ID,
        transitionId: TRANSITION_ID,
        fromGameVersion: "6.7",
        targetGameVersion: TARGET_GAME_VERSION,
        ...
        scope: {
            ...
            externalClaimFields: EXTERNAL_CLAIM_FIELDS,
            candidateSelection: "next 100 sorted ready behaviorSpec candidates after registered shard01, shard02, and shard03, fixed by persisted source-audit candidate IDs",
...
function validateSourceAudit(audit, { compareCurrent = false } = {}) {
    ...
    if (!audit || audit.kind !== "genshinR2BehaviorSpecShard04SourceAudit") return { valid: false, reasons: ["artifactMissing"] };
...
function validateAudit(audit, { compareCurrent = false } = {}) {
    const reasons = [];
    const add = (reason) => { if (!reasons.includes(reason)) reasons.push(reason); };
    if (!audit || audit.kind !== "genshinR2BehaviorSpecShard04G06Deferral") return { valid: false, reasons: ["artifactMissing"] };
...
function renderSourceAudit(audit) {
    ...
    return `# Genshin r2 BehaviorSpec shard04 source audit\n\n` +
        ...
        `Shard04 reuses the persisted 6.7/7.0 behavior snapshot. Entity raw records are evidence of identity and bytes only; prose, placeholders, or correlated entity data are not converted into candidate mechanic values.\n`;
}

function renderMarkdown(audit) {
    const summary = audit.summary || {};
    return `# Genshin r2 BehaviorSpec shard04 G06 deferral\n\n` +
