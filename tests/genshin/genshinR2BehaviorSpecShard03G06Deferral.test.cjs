...
const shard = require("../../scripts/genshinR2BehaviorSpecShard03G06Deferral.cjs");
...
test("shard03 selects exactly the next 100 ready BehaviorSpec candidates after shard01 and shard02", () => {
    ...
    assert.equal(sourceAudit.scope.candidateIds[0], "behavior:10000032:constellation:constellation-4");
    assert.equal(sourceAudit.scope.candidateIds.at(-1), "behavior:10000042:constellation:constellation-4");
...
test("shard03 source and deferral artifacts validate and remain fail-closed", () => {
    const sourceAudit = readJson(shard.SOURCE_AUDIT_PATH);
...
test("shard03 records raw readiness but no candidate value inference", () => {
    const sourceAudit = readJson(shard.SOURCE_AUDIT_PATH);
...
test("shard03 validation rejects a fail-open mutation", () => {
    const audit = readJson(shard.ARTIFACT_PATH);
...
test("shard03 artifact and source audit are deterministic for the persisted candidate set", () => {
    const sourceAudit = readJson(shard.SOURCE_AUDIT_PATH);
    const audit = readJson(shard.ARTIFACT_PATH);
...
test("shard03 decisions bind to the centrally allowed source-reopen task", () => {
    const audit = readJson(shard.ARTIFACT_PATH);
...
test("shard03 dedicated schemas are present and pin the bounded scope", () => {
    const deferralSchema = readJson(shard.SCHEMA_PATH);
    const sourceSchema = readJson("games/genshin/data/schema/r2-behavior-spec-shard03-source-audit.schema.json");
    assert.equal(deferralSchema.properties.kind.const, "genshinR2BehaviorSpecShard03G06Deferral");
    assert.equal(deferralSchema.$defs.scope.properties.candidateCount.const, 100);
    assert.equal(sourceSchema.properties.kind.const, "genshinR2BehaviorSpecShard03SourceAudit");
