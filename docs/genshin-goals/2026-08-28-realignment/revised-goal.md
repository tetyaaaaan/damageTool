...
2026-09-10承認の[原神v2データ作成標準](../../GENSHIN_V2_DATA_AUTHORING_STANDARD.md)を新規データ・既存データのv2化に適用する。[実行補足](execution-addendum-2026-09-06.md)のbatch/役割/同期手順と併用する。既存成果への一律遡及処理は要求しない。今回の統合履歴は[統合記録](authoring-standard-2026-09-10/integration-review.md)に保存する。
...
作成標準のcandidatePrepared / verificationComplete / runtimeConnectedは工程の到達点であり、以下の3軸や既存state machineを置き換えない。candidatePreparedは不足を記録した候補作成工程の終了であり、G06保留承認・goal処置完了ではない。必要なSpec確認と計算採用対象の独立検証を後続taskとして保持する。到達実績と現在のversion有効性を分ける。
...
Specの正本・単位・条件・依存関係は作成標準に従う。EffectSpecとBehaviorSpecの不可分な結合をsourceRefsの共通性だけで保証せず、必要依存が未対応なら該当Runtimeをblockedにする。明示された現在状態の単発計算と時系列自動再現を区別し、G07の部分有効化を維持する。標準の概念名をschemaが未対応のまま投入しない。
...
batch開始時に作成標準に沿ってcandidatePrepared・verificationComplete・runtimeConnectedまたは承認済みlegacy保守の終了地点を固定する。candidate作成の終了を必要な比較・修正・受入の終了へ読み替えない。既存の役割分担と実行補足を維持する。
...
作成標準の3到達点は対象field/version・現在有効性を添えた工程別報告とし、以下の既存KPIへ混算しない。新しい計測基盤や全件Runtime化を要求しない。G06の保留条件およびG13の8完了条件は本標準統合でも変更しない。
