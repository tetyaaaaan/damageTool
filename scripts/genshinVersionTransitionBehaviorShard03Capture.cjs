...
/**
 * Materialize the third authoritative behaviorSpec queue shard.
 *
 * This is deliberately a source-capture worker.  It records the pinned
 * genshin-db talents/constellations bytes and a deterministic 6.7 -> 7.0
 * comparison, but it does not parse prose into a behavior field, choose a
 * Runtime mapping, issue a certificate, or promote canonical data.
 *
 * The current queue names this shard
 * `behavior:behaviorSpec:semanticDecisionRequired:standard:03`.  The caller
 * requested the source-missing lane; the observed queue lane is persisted so
 * a later queue regeneration cannot silently change the source of truth.
 */
...
const providerRoot = path.join(transitionRoot, "sources", "genshin-db-shard03");
const queuePath = path.join(repositoryRoot, "reports", "genshin-evidence-task-queue.json");
const outputPath = path.join(transitionRoot, "behavior-shard-03-snapshot.json");
const checkpointPath = path.join(transitionRoot, "behavior-shard-03-capture-checkpoint.json");
...
const requestedQueuePrimaryBlockReason = "sourceMissing";
const shardOrdinal = "03";
const shardIdPattern = /^behavior:behaviorSpec:[^:]+:standard:03$/;
...
// The local entity ID is not present in genshin-db's talent/constellation
// record (the provider's top-level id is an internal record id).  These
// provider slugs are retained only as path identities and are verified by
// the fetched English `name` field before any bytes are accepted.
const resolvedEntities = [
    { entityId: "10000029", name: "Klee", nameJa: "クレー", slug: "klee" },
    { entityId: "10000030", name: "Zhongli", nameJa: "鍾離", slug: "zhongli" },
    { entityId: "10000031", name: "Fischl", nameJa: "フィッシュル", slug: "fischl" },
    { entityId: "10000032", name: "Bennett", nameJa: "ベネット", slug: "bennett" },
    { entityId: "10000033", name: "Tartaglia", nameJa: "タルタリヤ", slug: "tartaglia" },
    { entityId: "10000034", name: "Noelle", nameJa: "ノエル", slug: "noelle" },
    { entityId: "10000035", name: "Qiqi", nameJa: "七七", slug: "qiqi" },
    { entityId: "10000036", name: "Chongyun", nameJa: "重雲", slug: "chongyun" }
];
...
function queueShard() {
    ...
    const shard = shards.find((candidate) => shardIdPattern.test(String(candidate?.shardId || "")));
    if (!shard || !Array.isArray(shard.candidateIds) || shard.candidateIds.length === 0) {
        throw new Error(`authoritative behaviorSpec shard 03 missing: ${shardIdPattern}`);
    }
    ...
    if (new Set(candidateIds).size !== candidateIds.length || !candidateIds.every((id) => /^behavior:100000\d+:/.test(id))) {
        throw new Error("authoritative behaviorSpec shard 03 candidate IDs are malformed or duplicated");
    }
    const entityIds = [...new Set(candidateIds.map((candidateId) => candidateId.split(":")[1]))];
    if (entityIds.length !== resolvedEntities.length || entityIds.some((id) => !resolvedEntities.some((entity) => entity.entityId === id))) {
        throw new Error(`shard 03 entity inventory mismatch: ${entityIds.join(",")}`);
    }
    const observedReason = String(shard.shardId).split(":")[2];
    return {
        shardId: String(shard.shardId),
        queuePath: queuePathRelative,
        queueStatus: observedReason,
        requestedPrimaryBlockReason: requestedQueuePrimaryBlockReason,
        observedPrimaryBlockReason: observedReason,
        candidateIds,
...
async function fetchBytes(url) {
    const response = await fetch(url, { headers: { "User-Agent": "damageTool-genshin-behavior-shard-03" } });
    if (!response.ok) {
...
function recordComparison(before, after) {
    const beforeFile = path.resolve(repositoryRoot, before.path);
    const afterFile = path.resolve(repositoryRoot, after.path);
    const beforeValue = readJson(beforeFile);
    const afterValue = readJson(afterFile);
...
function buildSnapshot() {
    ...
    return {
        schemaVersion: 1,
        kind: "genshinVersionTransitionBehaviorShard03Snapshot",
        generatedAt,
        claim,
        summary: {
        ...
        gateEligibility: {
            ...
            canIssueEligibilityCertificate: false,
            canPromoteCanonical: false,
            reasons: ["providerIndependenceInsufficient", "behaviorSemanticsNotParsed", "providerRecordDoesNotEmbedEntityId"]
        },
...
function buildCheckpoint(snapshot = buildSnapshot()) {
    ...
    const revisionProgress = (revision, completedArtifactCount) => ({
        gameVersion: revision.gameVersion,
        revision: revision.revision,
        status: "complete",
    ...
    const claim = {
        ...
        requestedPrimaryBlockReason: requestedQueuePrimaryBlockReason,
        observedPrimaryBlockReason: snapshot.claim.queueShard.observedPrimaryBlockReason,
    ...
    return {
        schemaVersion: 1,
        kind: "genshinVersionTransitionBehaviorShard03CaptureCheckpoint",
        generatedAt,
        transitionId: "genshin:6.7->7.0",
        status: "complete",
        ...
        queueShard: {
            ...
            candidateCount: snapshot.claim.queueShard.authoritativeCandidateCount,
            candidateIdsDigest: snapshot.claim.queueShard.authoritativeCandidateIdsDigest,
            observedPrimaryBlockReason: snapshot.claim.queueShard.observedPrimaryBlockReason
        },
        revisions: {
            before: revisionProgress(revisions.before, ids.length * recordKinds.length),
            after: revisionProgress(revisions.after, ids.length * recordKinds.length)
        },
        nextTask: "Independent version-bound field evidence and behavior-semantic reconciliation remain required; no automatic promotion.",
        blocker: {
...
function validateArtifact(candidate, revision, entity, recordKind, root = repositoryRoot) {
    ...
    const bytes = fs.readFileSync(file);
    let value;
    try { value = assertEntityRecord(entity, recordKind, bytes); } catch (error) { reasons.push(`identityMismatch:${error.message}`); }
...
function validateBehaviorShard03Snapshot(snapshot, { repositoryRoot: root = repositoryRoot } = {}) {
    const reasons = [];
    if (snapshot?.schemaVersion !== 1 || snapshot?.kind !== "genshinVersionTransitionBehaviorShard03Snapshot") reasons.push("identityInvalid");
    ...
    if (snapshot?.claim?.provider !== provider || snapshot?.claim?.sourceFamily !== sourceFamily || snapshot?.claim?.repository !== repository) reasons.push("providerBindingInvalid");
    if (snapshot?.claim?.sourceFamilyCount !== 1) reasons.push("sourceFamilyCountInvalid");
    const queue = queueShard();
    ...
    if (!shard || shard.shardId !== queue.shardId
        ...
        || shard.requestedPrimaryBlockReason !== requestedQueuePrimaryBlockReason
        || shard.observedPrimaryBlockReason !== queue.observedPrimaryBlockReason) reasons.push("authoritativeShardBindingInvalid");
    ...
    for (const revision of Object.values(revisions)) {
        ...
        } catch (error) { reasons.push(`packageJsonInvalid:${revision.gameVersion}:${error.message}`); }
    }
...
function validateBehaviorShard03Checkpoint(checkpoint, { repositoryRoot: root = repositoryRoot } = {}) {
    const reasons = [];
    if (checkpoint?.schemaVersion !== 1 || checkpoint?.kind !== "genshinVersionTransitionBehaviorShard03CaptureCheckpoint") reasons.push("identityInvalid");
    ...
    const queue = queueShard();
    if (checkpoint?.requestedEntityCount !== queue.entityCount || digestStable(checkpoint?.requestedEntityIds) !== queue.entityIdsDigest) reasons.push("entityInventoryInvalid");
    if (checkpoint?.queueShard?.shardId !== queue.shardId || checkpoint?.queueShard?.candidateCount !== queue.candidateCount || checkpoint?.queueShard?.candidateIdsDigest !== queue.candidateIdsDigest) reasons.push("queueShardInvalid");
    for (const revision of Object.values(revisions)) {
        const progress = checkpoint?.revisions?.[revision === revisions.before ? "before" : "after"];
        if (!progress || progress.gameVersion !== revision.gameVersion || progress.revision !== revision.revision || progress.status !== "complete"
            || digestStable(progress.completedEntityIds) !== queue.entityIdsDigest || progress.completedArtifactCount !== queue.entityCount * recordKinds.length) reasons.push(`revisionProgressInvalid:${revision.gameVersion}`);
    }
    ...
    if (!materializedPath || !fs.existsSync(materializedPath)) reasons.push("materializedSnapshotMissing");
        ...
        if (checkpoint.materializedSnapshot.fieldDigest !== snapshot.fieldDigest || validateBehaviorShard03Snapshot(snapshot, { repositoryRoot: root }).valid !== true) reasons.push("materializedSnapshotInvalid");
    }
...
module.exports = {
    ...
    revisions,
    shardIdPattern,
    validateBehaviorShard03Checkpoint,
    validateBehaviorShard03Snapshot,
    writeSnapshot
};
