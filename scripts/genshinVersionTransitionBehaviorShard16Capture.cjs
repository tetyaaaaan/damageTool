...
/**
 * Capture the final authoritative behaviorSpec shard (16) for the
 * 6.7 -> 7.0 transition.  This is source evidence only: no prose is
 * translated, no eligibility certificate is issued, and no canonical data
 * is promoted.  Entity 10000131 is the overlap already owned by shard 15;
 * only 10000132 and 10000133 are materialized here.
 *
 * Local catalog/source-text/consumer-mapper identity is recorded exactly as
 * observed.  Missing mapper entries are evidence (not inferred names), while
 * a conflicting mapper entry fails closed before acquisition.
 */
...
const sourceNamespace = "genshin-db-behavior-shard16";
...
const outputPath = path.join(transitionRoot, "behavior-shard-16-snapshot.json");
const checkpointPath = path.join(transitionRoot, "behavior-shard-16-capture-checkpoint.json");
...
const shardId = "behavior:behaviorSpec:semanticDecisionRequired:standard:16";
const expectedCandidateCount = 33;
const expectedCandidateIdsDigest = "9029efb1276280eea2b547008c870948424a834a57242caa3733ee55ee248e5d";
const expectedEntityIds = ["10000131", "10000132", "10000133"];
const expectedEntityIdsDigest = "447ca314d38783f32a0090e4ca5cdff9d169f88f45dac3559e700012cb5af56c";
const overlapEntityIds = ["10000131"];
...
// Resolved from concrete pinned English-tree paths in both revisions.  The
// provider calls 10000132 "Prune" while the local Japanese catalog is the
// authority for the display name プルーネ.
const resolvedEntities = [
    { entityId: "10000132", name: "Prune", nameJa: "プルーネ", slug: "prune", slugCandidates: ["prune"], providerId: 13201 },
    { entityId: "10000133", name: "Sandrone", nameJa: "サンドローネ", slug: "sandrone", slugCandidates: ["sandrone"], providerId: 13301 }
];
...
function localIdentityEvidence(root = repositoryRoot) {
    ...
    const entities = resolvedEntities.map((entity) => {
        ...
        if (!catalogRecord || catalogRecord.nameJa !== entity.nameJa) throw new Error(`local catalog identity mismatch for ${entity.entityId}: expected ${entity.nameJa}, got ${catalogRecord?.nameJa || "missing"}`);
...
function queueShard() {
    ...
    if (candidateIds.length !== expectedCandidateCount || new Set(candidateIds).size !== candidateIds.length || !candidateIds.every((id) => /^behavior:100\d{5}:[^:]+:.+$/.test(id))) {
        throw new Error(`authoritative behaviorSpec shard 16 candidate inventory invalid: ${candidateIds.length}`);
    }
    const candidateIdsDigest = digestStable(candidateIds);
    if (candidateIdsDigest !== expectedCandidateIdsDigest) throw new Error(`authoritative behaviorSpec shard 16 candidate digest mismatch: expected ${expectedCandidateIdsDigest}, got ${candidateIdsDigest}`);
    const entityIds = [...new Set(candidateIds.map((candidateId) => candidateId.split(":")[1]))];
    if (digestStable(entityIds) !== expectedEntityIdsDigest) throw new Error(`shard 16 entity inventory mismatch: expected ${expectedEntityIds.join(",")}, got ${entityIds.join(",")}`);
    if (entityIds.some((entityId) => travelerEntityIds.includes(entityId))) throw new Error("Traveler identity must remain fail-closed");
    return { shardId, queuePath: relative(queuePath), candidateIds, candidateIdsDigest, entityIds, entityIdsDigest: digestStable(entityIds), candidateCount: candidateIds.length, entityCount: entityIds.length };
}

async function fetchBytes(url) {
    const response = await fetch(url, { headers: { "User-Agent": "damageTool-genshin-behavior-shard-16" } });
    if (!response.ok) {
...
async function acquire() {
    ...
    return { candidateCount: expectedCandidateCount, entityCount: expectedEntityIds.length };
}
...
function buildEntity(entity, identity, root = repositoryRoot) {
    ...
    return {
        ...
        identityResolution: { status: "resolvedByLocalCatalogAndProviderName", localIdentityEvidenceDigest: digestStable(identity), providerNameMatches: true, providerEmbeddedIdMatches: true },
        treeResolution: { method: "nonRecursiveGitTreePath", resolutionEvidence: "immutableRawPathWithEmbeddedId", directoryPaths: recordKinds.map((recordKind) => `src/data/English/${recordKind}`), pathResolution: recordKinds.map((recordKind) => ({ recordKind, path: `src/data/English/${recordKind}/${entity.slug}.json` })) },
        records
    };
}
...
function buildSnapshot(root = repositoryRoot) {
    ...
    if (skipped.join(",") !== overlapEntityIds.join(",")) throw new Error(`shard 16 overlap mismatch: ${skipped.join(",")}`);
    const claim = {
        ...
        sourceNamespace,
        repository,
        treeResolution: { method: "nonRecursiveGitTreePath", resolutionEvidence: "immutableRawPathWithEmbeddedId", note: "Each concrete path is accepted only when both pinned revisions contain the expected provider name and explicit $.id." },
        ...
        shard: {
            ...
            overlapPolicy: "alreadyCapturedByBehaviorShard15",
            overlapEvidence: [{ entityId: "10000131", previousShardId: "behavior:behaviorSpec:semanticDecisionRequired:standard:15", acquisitionPolicy: "notReacquired" }]
        },
        ...
        variantAmbiguities: [],
        travelerVariantPolicy: { status: "notApplicable", failClosedOnEncounter: true, entityIds: travelerEntityIds, note: "No Traveler entity is in shard16; any future Traveler queue entry must remain unresolved until its variant identity is explicit." },
        coverage: {
    ...
    return {
        ...
        generatedAt,
        claim,
        summary: { shardId, authoritativeCandidates: queue.candidateCount, authoritativeEntities: queue.entityCount, resolvedEntities: entities.length, overlapSkippedEntities: overlapEntityIds.length, ambiguousEntities: 0, resolvedRecords: records.length, changedRecords: changedRecords.length, changedFields, sourceFamilyCount: 1, certificateEligibleClaims: 0 },
        gateEligibility: { status: "singleCorrelatedFamilyFailClosed", sourceFamilyCount: 1, canIssueEligibilityCertificate: false, canPromoteCanonical: false, reasons: ["providerIndependenceInsufficient", "behaviorSemanticsNotParsed"] },
...
function validateBehaviorShard16Snapshot(snapshot, { repositoryRoot: root = repositoryRoot } = {}) {
    const reasons = [];
    ...
    for (const expected of resolvedEntities) {
        ...
        if (!entity || entity.name !== expected.name || entity.nameJa !== expected.nameJa || entity.providerSlug !== expected.slug || entity.providerId !== expected.providerId || entity.resolutionStatus !== "resolvedUnambiguous" || entity.identityResolution?.status !== "resolvedByLocalCatalogAndProviderName") { reasons.push(`resolvedEntityInvalid:${expected.entityId}`); continue; }
        ...
        for (const recordKind of recordKinds) {
            ...
            if (!record.before?.path || !record.after?.path || !fs.existsSync(path.resolve(root, record.before.path)) || !fs.existsSync(path.resolve(root, record.after.path))) continue;
...
function buildCheckpoint(snapshot = buildSnapshot()) {
    ...
    return {
        ...
        revisions: Object.fromEntries(Object.entries(revisions).map(([side, revision]) => [side, { gameVersion: revision.gameVersion, revision: revision.revision, status: "complete", completedEntityIds: resolvedEntities.map((entity) => entity.entityId), completedArtifactCount: resolvedEntities.length * recordKinds.length }])),
        materializedSnapshot: { path: relative(outputPath), fieldDigest: snapshot.fieldDigest, entityCount: snapshot.summary.resolvedEntities, recordCount: snapshot.summary.resolvedRecords, changedRecordCount: snapshot.summary.changedRecords, changedFieldCount: snapshot.summary.changedFields },
        nextTask: "Reconcile this correlated capture with an independent strictly 7.0-bound field source; do not promote from this source family.",
...
function validateCheckpoint(checkpoint, { repositoryRoot: root = repositoryRoot } = {}) {
    ...
    if (!materializedPath || !fs.existsSync(materializedPath)) reasons.push("materializedSnapshotMissing");
        ...
        if (snapshot && (checkpoint.materializedSnapshot.fieldDigest !== snapshot.fieldDigest || !validateBehaviorShard16Snapshot(snapshot, { repositoryRoot: root }).valid)) reasons.push("materializedSnapshotInvalid");
    }
...
module.exports = {
    ...
    shardId,
    sourceNamespace,
    validateBehaviorShard16Snapshot,
    validateCheckpoint,
    writeSnapshot
};
