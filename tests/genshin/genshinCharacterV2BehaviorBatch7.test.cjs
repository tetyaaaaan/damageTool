...
const {
    ...
    defaultOutputRoot,
    stableJson
} = require("../../scripts/genshinCharacterV2BehaviorBatch7Generate.cjs");
const { auditBehaviorBatch7 } = require("../../scripts/genshinCharacterV2BehaviorBatch7Audit.cjs");
...
test("batch 7 fixes the requested ten non-pilot character IDs", () => {
    ...
    assert.equal(artifact.summary.inventoryInputCandidates, 181);
    assert.equal(artifact.summary.explicitCandidateOperations, 122);
    assert.equal(artifact.summary.unknownCandidateIds, 59);
    assert.equal(artifact.summary.sourceInventoryCandidates, 181);
    assert.equal(artifact.summary.explicitInventoryCandidates, 122);
    assert.equal(artifact.summary.unknownInventoryCandidates, 59);
...
test("raw source pointers and behavior-inventory candidates resolve without inference", () => {
    ...
    assert.equal(explicitCount, 122);
    assert.equal(unknownCount, 59);
    assert.deepEqual(artifact.modifiers, {});
});

test("batch 7 generation, split files, and audit are deterministic", () => {
    ...
    const report = auditBehaviorBatch7({ dataRoot: defaultDataRoot, outputRoot: defaultOutputRoot });
