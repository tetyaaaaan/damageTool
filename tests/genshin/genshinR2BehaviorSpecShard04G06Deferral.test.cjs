...
const shard = require("../../scripts/genshinR2BehaviorSpecShard04G06Deferral.cjs");
...
test("shard04 selects exactly the next 100 ready BehaviorSpec candidates after shard01 through shard03", () => {
    ...
    const shard02 = readJson("games/genshin/data/v2/reviews/r2-behavior-spec-shard02-source-audit.json");
    const shard03 = readJson("games/genshin/data/v2/reviews/r2-behavior-spec-shard03-source-audit.json");
    assert.equal(sourceAudit.scope.candidateCount, 100);
    assert.equal(new Set(sourceAudit.scope.candidateIds).size, 100);
    assert.equal(sourceAudit.scope.candidateIds[0], "behavior:10000042:constellation:constellation-5");
    assert.equal(sourceAudit.scope.candidateIds.at(-1), "behavior:10000050:constellation:constellation-6");
    assert.equal(sourceAudit.scope.candidateIds.some((id) => shard01.scope.candidateIds.includes(id)), false);
    assert.equal(sourceAudit.scope.candidateIds.some((id) => shard02.scope.candidateIds.includes(id)), false);
    assert.equal(sourceAudit.scope.candidateIds.some((id) => shard03.scope.candidateIds.includes(id)), false);
...
test("shard04 source and deferral artifacts validate and remain fail-closed", () => {
    const sourceAudit = readJson(shard.SOURCE_AUDIT_PATH);
...
test("shard04 records raw readiness but no candidate value inference", () => {
    const sourceAudit = readJson(shard.SOURCE_AUDIT_PATH);
...
test("shard04 validation rejects a fail-open mutation", () => {
    const audit = readJson(shard.ARTIFACT_PATH);
...
test("shard04 artifact and source audit are deterministic for the persisted candidate set", () => {
    const sourceAudit = readJson(shard.SOURCE_AUDIT_PATH);
    const audit = readJson(shard.ARTIFACT_PATH);
...
test("shard04 decisions bind to the projected source-reopen task and remain centrally fail-closed until registered", () => {
    const audit = readJson(shard.ARTIFACT_PATH);
    ...
    assert.deepEqual(work.validateDeferral(decision, task, { targetGameVersion: "7.0" }), { valid: false, errors: ["concreteWorkStillPending"] });
});

test("shard04 dedicated schemas are present and pin the bounded scope", () => {
    const deferralSchema = readJson(shard.SCHEMA_PATH);
    const sourceSchema = readJson("games/genshin/data/schema/r2-behavior-spec-shard04-source-audit.schema.json");
    assert.equal(deferralSchema.properties.kind.const, "genshinR2BehaviorSpecShard04G06Deferral");
    assert.equal(deferralSchema.$defs.scope.properties.candidateCount.const, 100);
    assert.equal(sourceSchema.properties.kind.const, "genshinR2BehaviorSpecShard04SourceAudit");
