const test = require("node:test");
const assert = require("node:assert/strict");
const { buildAudit, renderMarkdown } = require("../../scripts/genshinModifierAudit.cjs");

test("STEP 19 audit classifies every modifier with a stable reason code", () => {
    const audit = buildAudit();
    assert.equal(audit.summary.total, 1561);
    assert.equal(audit.records.filter((record) => {
        return ["unsupported", "invalidData", "missingInput", "displayOnly"].includes(record.supportStatus)
            && !record.reasonCode;
    }).length, 0);
    assert.equal(audit.records.filter((record) => record.lane === "review").length, 0);
    assert.equal((audit.summary.byPriority.P0 || 0) + (audit.summary.byPriority.P1 || 0) + (audit.summary.byPriority.P2 || 0), 0);
    assert.equal(audit.summary.byReasonCode.DEDICATED_FORMULA_DEFERRED, 14);
    assert.equal(audit.summary.byPriority.P3, 14);
});

test("STEP 19 audit output is deterministic and separates input/display records", () => {
    const first = buildAudit();
    const second = buildAudit();
    assert.deepEqual(first, second);
    assert.equal(first.summary.byLane.interactiveInput, 68);
    assert.equal(first.records.filter((record) => record.inputImplemented).length, 68);
    assert.equal((first.summary.bySchemaStatus.canonical || 0) + (first.summary.bySchemaStatus.legacyCompatible || 0), first.summary.total);
    assert.equal(first.summary.byReasonCode.DISPLAY_ONLY_SOURCE_TEXT, 15);
    assert.match(renderMarkdown(first), /原神補正監査レポート/);
});

test("committed audit report matches the current modifier data", () => {
    const committed = require("../../reports/genshin-modifier-audit.json");
    assert.deepEqual(buildAudit().summary, committed.summary);
});

test("stack inputs have complete ranges consistent with their modifier contract", () => {
    const audit = buildAudit();
    assert.deepEqual(audit.contractViolations, []);
});
