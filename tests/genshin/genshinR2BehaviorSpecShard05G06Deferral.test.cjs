...
const shard = require("../../scripts/genshinR2BehaviorSpecShard05G06Deferral.cjs");
...
test("shard05 selects exactly the next 100 ready BehaviorSpec candidates after shards01 through04", () => {
    const source = readJson(shard.SOURCE_AUDIT_PATH);
    const prior = [1, 2, 3, 4].map((n) => readJson(`games/genshin/data/v2/reviews/r2-behavior-spec-shard0${n}-source-audit.json`));
    assert.equal(source.scope.candidateCount, 100);
    assert.equal(new Set(source.scope.candidateIds).size, 100);
    assert.equal(source.scope.candidateIds[0], "behavior:10000050:talent:burst");
    assert.equal(source.scope.candidateIds.at(-1), "behavior:10000057:talent:normalattack-charged");
...
test("shard05 source and deferral artifacts validate and remain fail-closed", () => {
    const source = readJson(shard.SOURCE_AUDIT_PATH);
...
test("shard05 records verified raw entity evidence without candidate value inference", () => {
    const source = readJson(shard.SOURCE_AUDIT_PATH);
...
test("shard05 validation rejects a fail-open certificate mutation", () => {
    const audit = readJson(shard.ARTIFACT_PATH);
...
test("shard05 source and deferral generation is deterministic for the persisted scope", () => {
    const source = readJson(shard.SOURCE_AUDIT_PATH);
    const audit = readJson(shard.ARTIFACT_PATH);
...
test("shard05 decisions retain immutable evidence references and central validation stays fail-closed before registration", () => {
    const audit = readJson(shard.ARTIFACT_PATH);
    const decision = audit.decisions[0];
    const task = { candidateId: decision.candidateId, taskId: decision.candidateId, layer: "behaviorSpec", dataset: "behaviorBatch5", status: "ready", deferredLanes: decision.assessment.boundedScope.deferredLaneDispositions };
...
test("shard05 schemas and frontier are pinned to the exact bounded scope", () => {
    const schema = readJson(shard.SCHEMA_PATH);
    const sourceSchema = readJson("games/genshin/data/schema/r2-behavior-spec-shard05-source-audit.schema.json");
    const audit = readJson(shard.ARTIFACT_PATH);
    assert.equal(schema.properties.kind.const, "genshinR2BehaviorSpecShard05G06Deferral");
    assert.equal(schema.$defs.scope.properties.candidateCount.const, 100);
    assert.equal(sourceSchema.properties.kind.const, "genshinR2BehaviorSpecShard05SourceAudit");
