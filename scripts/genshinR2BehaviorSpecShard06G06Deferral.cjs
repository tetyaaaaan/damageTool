...
/**
 * Candidate-scoped r2 audit for BehaviorSpec shard06.
 *
 * This shard consumes only the already materialized 6.7/7.0 entity records.
 * It records an exact, reopenable wait for the persisted provider surface;
 * it does not infer mechanics from prose, mutate the queue, or activate a
...
const SOURCE_REGISTRY_PATH = path.join(ROOT, "games", "genshin", "data", "v2", "source-search-frontiers.json");
const SNAPSHOT_PATH = path.join(ROOT, "games", "genshin", "data", "v2", "version-transitions", "6.7-to-7.0", "behavior-shard-06-snapshot.json");
const SOURCE_AUDIT_PATH = path.join(ROOT, "games", "genshin", "data", "v2", "reviews", "r2-behavior-spec-shard06-source-audit.json");
const SOURCE_AUDIT_REPORT_PATH = path.join(ROOT, "reports", "genshin-r2-behavior-spec-shard06-source-audit.md");
const ARTIFACT_PATH = path.join(ROOT, "games", "genshin", "data", "v2", "reviews", "r2-behavior-spec-shard06-g06-deferral.json");
const REPORT_PATH = path.join(ROOT, "reports", "genshin-r2-behavior-spec-shard06-g06-deferral.md");
const SCHEMA_PATH = path.join(ROOT, "games", "genshin", "data", "schema", "r2-behavior-spec-shard06-g06-deferral.schema.json");
...
const SHARD_ID = "behavior:behaviorSpec:semanticDecisionRequired:standard:06";
const FRONTIER_ID = "genshin-7.0-behavior-spec-shard06-persisted-provider-frontier";
const FRONTIER_DATE = "2026-08-31";
...
const PRIOR_SOURCE_AUDITS = [1, 2, 3, 4, 5].map((n) => path.join(ROOT, "games", "genshin", "data", "v2", "reviews", `r2-behavior-spec-shard0${n}-source-audit.json`));
...
function candidateIdDigest(ids) { return digestStable(sortedUnique(ids)); }
function taskDigest(task) { return work.taskDigest(task); }
function withoutDigest(value) { const copy = clone(value); if (copy && typeof copy === "object") delete copy.fieldDigest; return copy; }
...
function persistedShardIds() {
    const ids = new Set();
    for (const file of PRIOR_SOURCE_AUDITS) {
        if (!fs.existsSync(file)) continue;
        for (const id of readJson(file).scope?.candidateIds || []) ids.add(String(id));
    }
    return ids;
}
...
function selectedIds(materialization) {
    ...
    const next = eligibleCandidates(materialization).filter((candidate) => !excluded.has(String(candidate.candidateId))).slice(0, SHARD_SIZE);
...
function sourceClaim(candidate, field) { return (candidate.candidateClaims || []).find((claim) => claim.field === field) || {}; }
...
function providerSurface(sourceSearch, policy, rawEntityCount) {
...
function buildSourceAudit() {
    ...
    const surface = providerSurface(sourceSearch, policy, selected.length);
    const candidates = selected.map((candidate, index) => normalizeCandidate(candidate, index + 1, surface.providers));
    ...
    const output = {
        schemaVersion: 1,
        kind: "genshinR2BehaviorSpecShard06SourceAudit",
        status: "passed",
        generatedAt: GENERATED_AT,
        generator: { name: path.basename(__filename), version: "genshinR2BehaviorSpecShard06SourceAudit/1" },
        scope: {
            transitionId: TRANSITION_ID,
            targetGameVersion: TARGET_GAME_VERSION,
            layer: "behaviorSpec",
            shardId: SHARD_ID,
            selection: "next 100 sorted ready BehaviorSpec tasks after the registered shard01 through shard05, with resolved identity, verified transition raw, and complete local claim schema",
        ...
        summary: {
            ...
            identityGapCandidateCount: 0,
            localClaimSchemaGapCandidateCount: 0,
            rawIntegrityGapCandidateCount: 0,
            ...
            sourceSearchProviderCount: surface.providerCount,
...
function sourceAuditBytes(audit) { return Buffer.from(`${JSON.stringify(audit, null, 2)}\n`, "utf8"); }
function sourceAuditRef(audit) { return { path: relative(SOURCE_AUDIT_PATH), sha256: sha256(sourceAuditBytes(audit)) }; }
function immutableRefs(sourceAudit) {
    return [sourceAuditRef(sourceAudit), fileRef(SNAPSHOT_PATH, "transitionBehaviorSnapshot"), fileRef(SOURCE_SEARCH_PATH, "transitionProviderSearchLedger"), fileRef(PROVIDER_POLICY_PATH, "providerIndependencePolicy"), fileRef(SOURCE_FAMILY_PATH, "sourceFamilyRegistry"), fileRef(EXTERNAL_CHARACTER_PATH, "characterExternalFieldEvidence")];
}
...
function decisionFor(candidate, refs, frontier, projectedTask) {
    ...
    const lane = { kind: "semanticDecision", blockingReasons: ["semanticDecisionRequired"], disposition: "externalEvidenceWait", reason, fieldRefs: fieldFindings.map((item) => item.field), artifactRefs: refs.map((ref) => ({ path: ref.path, sha256: ref.sha256 })) };
    return {
        id: `r2-behavior-shard06-g06-deferral:${candidate.candidateId}`,
        kind: "genshinEvidenceDeferral",
        policyId: POLICY_ID,
        candidateId: candidate.candidateId,
        targetGameVersion: TARGET_GAME_VERSION,
        queueTaskDigest: taskDigest(projectedTask),
        assessment: {
            actorId: "luna_worker",
            ...
            boundedScope: {
                candidateId: candidate.candidateId,
                taskId: projectedTask?.task?.taskId || null,
                remainingSearches: clone(UNSEARCHED_SCOPE),
                notExecutableReasons: ["strictIndependentTargetFieldEvidenceMissing", "providerOwnedCandidateFieldArtifactNotPersisted", "futureProviderSurfaceOutsideFiniteFrontier"],
        ...
        draftDisposition: { status: "externalEvidenceWait", evidenceDeferred: true, workClosed: false, strictVerified: false, certificateEligible: false, canonicalPromotionEligible: false, queueMutation: "none" }
    };
}
...
function frontierFor(sourceAudit) {
    ...
    const frontier = {
        ...
        claimFields: CLAIM_FIELDS,
        externalClaimFields: EXTERNAL_CLAIM_FIELDS,
        searchScope: `Exact ${ids.length} behaviorSpec candidates in ${SHARD_ID}; immutable provider artifacts and prerequisite ledgers retained in this repository at audit time.`,
    ...
    frontier.digest = digestStable({ id: frontier.id, status: frontier.status, finite: frontier.finite, shardId: frontier.shardId, candidateIds: frontier.candidateIds, claimFields: frontier.claimFields, externalClaimFields: frontier.externalClaimFields, searchScope: frontier.searchScope, lastSearchedAt: frontier.lastSearchedAt, providersExamined: frontier.providersExamined, negativeResult: frontier.negativeResult, unsearchedScope: frontier.unsearchedScope, reopenTrigger: frontier.reopenTrigger, sourceAuditDigest: frontier.sourceAuditDigest });
    return frontier;
}
function frontierEntry(frontier, refs) {
    return { id: frontier.id, status: frontier.status, appliesTo: { dataset: "behavior", layer: "behaviorSpec", candidateCount: frontier.candidateCount, candidateIdDigest: frontier.candidateIdDigest, candidateIds: clone(frontier.candidateIds) }, searchScope: frontier.searchScope, lastSearchedAt: frontier.lastSearchedAt, providersExamined: clone(frontier.providersExamined), negativeResult: frontier.negativeResult, reopenTrigger: frontier.reopenTrigger, evidenceRefs: refs.map((ref) => ref.path), sourceAuditDigest: frontier.sourceAuditDigest };
}
...
function buildAudit() {
    ...
    const projectedRegistry = { ...clone(registry), frontiers: (registry.frontiers || []).filter((entry) => entry?.id !== FRONTIER_ID).concat(frontierEntry(frontier, refs)) };
    ...
    const materialization = readJson(MATERIALIZATION_PATH);
    const byId = new Map(eligibleCandidates(materialization).map((candidate) => [String(candidate.candidateId), candidate]));
    const candidates = sourceAudit.scope.candidateIds.map((id) => byId.get(String(id)));
    ...
    const candidateEvidence = sourceAudit.candidates.map((candidate) => ({
        ...
        queueTaskDigest: decisions.find((decision) => String(decision.candidateId) === String(candidate.candidateId)).queueTaskDigest,
        ...
        claimStatuses: EXTERNAL_CLAIM_FIELDS.map((field) => ({ field, status: "candidateFieldSearchExhaustedForPersistedSurface", candidateFieldValueMaterialized: false, independentVersionBound: false, strictVerified: false })),
        safety: { candidateActive: false, runtimeChanged: false, certificateIssued: false, canonicalPromoted: false }
    }));
    const output = {
        schemaVersion: 1,
        kind: "genshinR2BehaviorSpecShard06G06Deferral",
        policyId: POLICY_ID,
        transitionId: TRANSITION_ID,
        fromGameVersion: "6.7",
        targetGameVersion: TARGET_GAME_VERSION,
        ...
        draftOnly: true,
        generatedAt: GENERATED_AT,
        generator: { name: path.basename(__filename), version: "genshinR2BehaviorSpecShard06G06Deferral/1" },
        scope: { shardId: SHARD_ID, layer: "behaviorSpec", candidateCount: SHARD_SIZE, candidateIds: clone(sourceAudit.scope.candidateIds), candidateIdDigest: sourceAudit.scope.candidateIdDigest, externalClaimFields: EXTERNAL_CLAIM_FIELDS, candidateSelection: "next 100 sorted ready behaviorSpec candidates after registered shard01 through shard05, fixed by persisted source-audit candidate IDs", frontierId: FRONTIER_ID },
        queueObservation: { taskCount: SHARD_SIZE, candidateIdDigest: sourceAudit.scope.candidateIdDigest, note: "Queue is observed through candidate task digests for provenance only; this draft does not mutate authoritative task progress or status." },
        frontier,
        evidence: { sourceAudit: sourceAuditRef(sourceAudit), providerSurface: clone(sourceAudit.providerSurface), candidateEvidence, candidateFieldEvidenceCandidateCount: 0, candidateFieldEvidenceClaimCount: 0, independentVersionBoundFieldEvidenceClaimCount: 0, rawIntegrityVerifiedCandidateCount: sourceAudit.summary.rawIntegrityVerifiedCandidateCount, rawIntegrityGapCandidateCount: sourceAudit.summary.rawIntegrityGapCandidateCount, sourceRefs: refs.map((ref) => ({ path: ref.path, sha256: ref.sha256 })) },
        decisions,
        summary: { candidateCount: SHARD_SIZE, entityCount: sourceAudit.summary.entityCount, claimCount: SHARD_SIZE * CLAIM_FIELDS.length, externalClaimCount: SHARD_SIZE * EXTERNAL_CLAIM_FIELDS.length, internalClaimCount: SHARD_SIZE, candidateScopedFrontierProvenCandidateCount: SHARD_SIZE, finiteFrontierStatus: "searchExhaustedForPersistedProviderSurface", evidenceDeferredDraftCandidateCount: SHARD_SIZE, candidateFieldEvidenceCandidateCount: 0, candidateFieldEvidenceClaimCount: 0, strictVerifiedCandidateCount: 0, certificateEligibleClaimCount: 0, canonicalPromotionEligibleCandidateCount: 0, providerCount: sourceAudit.providerSurface.providerCount, providerExactCandidateFieldHitCount: 0, unsearchedExternalRemainderCandidateCount: SHARD_SIZE, queueMutated: false },
        gate: { draftEligibleForEvidenceWait: true, canIssueEligibilityCertificate: false, canPromoteCanonical: false, canDeferG06: true, verificationGranted: false, promotionGranted: false, runtimeChanged: false, certificateIssued: false, queueChanged: false, reason: "This draft closes only the finite persisted-provider search surface for a future reopenable evidence wait. It does not verify values, grant a certificate, activate Runtime, promote canonical data, or close queue work without an exact central policy integration." },
        policy: { candidateScopedFrontierRequired: true, persistedProviderSurfaceOnly: true, futureProviderSurfaceRemainsOpen: true, providerIndependenceRequired: true, explicitGameVersionBindingRequired: true, completeCandidateClaimCoverageRequired: true, proseInference: "forbidden", tokenMatching: "forbidden", sameUpstreamDoubleCount: "forbidden", strictVerification: "unchanged", queueMutation: "none", runtimeOrCanonicalMutation: "none" },
...
function validateSourceAudit(audit, { compareCurrent = false } = {}) {
    ...
    if (!audit || audit.kind !== "genshinR2BehaviorSpecShard06SourceAudit") return { valid: false, reasons: ["artifactMissing"] };
    ...
    if (audit.providerSurface?.providerCount !== 9 || audit.providerSurface?.candidateScopedFrontierProven !== false || audit.providerSurface?.exactCandidateFieldEvidenceCount !== 0 || audit.providerSurface?.independentVersionBoundFieldEvidenceCount !== 0) add("providerGateInvalid");
    if (candidateIdDigest(audit.scope?.candidateIds || []) !== audit.scope?.candidateIdDigest) add("candidateDigestInvalid");
    if (!Array.isArray(audit.candidates) || audit.candidates.length !== SHARD_SIZE) add("candidateCountInvalid");
    const seen = new Set();
    for (const candidate of audit.candidates || []) {
        if (!candidate?.candidateId || seen.has(candidate.candidateId) || !audit.scope.candidateIds.includes(candidate.candidateId)) add(`candidateInvalid:${candidate?.candidateId || "<missing>"}`);
...
function validateAudit(audit, { compareCurrent = false } = {}) {
    const reasons = [];
    const add = (reason) => { if (!reasons.includes(reason)) reasons.push(reason); };
    if (!audit || audit.kind !== "genshinR2BehaviorSpecShard06G06Deferral") return { valid: false, reasons: ["artifactMissing"] };
    ...
    for (const decision of audit.decisions || []) {
        ...
        const refs = [...(decision?.search?.artifactRefs || []), ...(decision?.safety?.artifactRefs || []), ...(decision?.assessment?.boundedScope?.artifactRefs || [])];
...
function writeArtifacts(audit = null) {
    ...
    // Persist the source audit before building the projected queue.  The
    // terminal validator requires every immutable evidenceRef to resolve on
    // disk; building the default audit in a parameter initializer would run
    // before this shard's own source-audit file exists and silently leave the
    // projected task in its pre-frontier source-search shape.
    const output = audit && audit.fieldDigest ? audit : buildAudit();
...
module.exports = { ARTIFACT_PATH, CLAIM_FIELDS, EXTERNAL_CLAIM_FIELDS, FRONTIER_ID, GENERATED_AT, REPORT_PATH, SCHEMA_PATH, SHARD_ID, SOURCE_AUDIT_PATH, SOURCE_AUDIT_REPORT_PATH, buildAudit, buildSourceAudit, candidateIdDigest, frontierEntry, frontierFor, immutableRefs, renderMarkdown, renderSourceAudit, selectedIds, validateAudit, validateSourceAudit, withoutDigest, writeArtifacts };
