...
const {
    ...
    defaultOutputRoot,
    stableJson
} = require("../../scripts/genshinCharacterV2BehaviorBatch8Generate.cjs");
const { auditBehaviorBatch8 } = require("../../scripts/genshinCharacterV2BehaviorBatch8Audit.cjs");
...
test("batch 8 fixes the requested ten non-pilot character IDs", () => {
    ...
    assert.equal(artifact.summary.inventoryInputCandidates, 209);
    assert.equal(artifact.summary.explicitCandidateOperations, 142);
    assert.equal(artifact.summary.unknownCandidateIds, 67);
    assert.equal(artifact.summary.sourceInventoryCandidates, 209);
    assert.equal(artifact.summary.explicitInventoryCandidates, 142);
...
test("raw source pointers and behavior-inventory candidates resolve without inference", () => {
    ...
    assert.equal(explicitCount, 142);
    assert.equal(unknownCount, 67);
    assert.deepEqual(artifact.modifiers, {});
});

test("batch 8 generation, split files, and audit are deterministic", () => {
    ...
    const report = auditBehaviorBatch8({ dataRoot: defaultDataRoot, outputRoot: defaultOutputRoot });
