...
/**
 * Capture the authoritative behaviorSpec shard 10 for the 6.7 -> 7.0
 ...
 * data.
 *
 * The provider's numeric ids for the last two entities are not a copy of
 * the local avatar ids: local 10000092 (Xianyun) is provider 9301 and local
 * 10000093 (Gaming) is provider 9201.  The mapping is accepted only after
 * the local catalog/source-text identity and both pinned provider records
 * agree by name and embedded id.  A future mismatch therefore fails closed.
 */
...
const sourceNamespace = "genshin-db-behavior-shard10";
...
const outputPath = path.join(transitionRoot, "behavior-shard-10-snapshot.json");
const checkpointPath = path.join(transitionRoot, "behavior-shard-10-capture-checkpoint.json");
...
const shardId = "behavior:behaviorSpec:semanticDecisionRequired:standard:10";
...
// revisions.  In particular, 10000092/93 deliberately retain the provider
// id swap proven by the provider name, rather than assuming id suffixes.
const resolvedEntities = [
    { entityId: "10000085", name: "Freminet", nameJa: "フレミネ", slug: "freminet", slugCandidates: ["freminet"], providerId: 8501 },
    { entityId: "10000086", name: "Wriothesley", nameJa: "リオセスリ", slug: "wriothesley", slugCandidates: ["wriothesley"], providerId: 8601 },
    { entityId: "10000087", name: "Neuvillette", nameJa: "ヌヴィレット", slug: "neuvillette", slugCandidates: ["neuvillette"], providerId: 8701 },
    { entityId: "10000088", name: "Charlotte", nameJa: "シャルロット", slug: "charlotte", slugCandidates: ["charlotte"], providerId: 8801 },
    { entityId: "10000089", name: "Furina", nameJa: "フリーナ", slug: "furina", slugCandidates: ["furina"], providerId: 8901 },
    { entityId: "10000090", name: "Chevreuse", nameJa: "シュヴルーズ", slug: "chevreuse", slugCandidates: ["chevreuse"], providerId: 9001 },
    { entityId: "10000091", name: "Navia", nameJa: "ナヴィア", slug: "navia", slugCandidates: ["navia"], providerId: 9101 },
    { entityId: "10000092", name: "Xianyun", nameJa: "閑雲", slug: "xianyun", slugCandidates: ["xianyun"], providerId: 9301 },
    { entityId: "10000093", name: "Gaming", nameJa: "嘉明", slug: "gaming", slugCandidates: ["gaming"], providerId: 9201 }
];
...
function localIdentityEvidence() {
    ...
    return {
        ...
        swappedProviderIdEvidence: [
            { entityId: "10000092", localName: "Xianyun", providerSlug: "xianyun", providerId: 9301, reason: "provider embedded id is 9301; name is Xianyun" },
            { entityId: "10000093", localName: "Gaming", providerSlug: "gaming", providerId: 9201, reason: "provider embedded id is 9201; name is Gaming" }
        ]
    };
}
...
function queueShard() {
    ...
    if (candidateIds.length !== 100 || new Set(candidateIds).size !== candidateIds.length || !candidateIds.every((id) => /^behavior:100000\d{2}:[^:]+:.+$/.test(id))) {
        throw new Error(`authoritative behaviorSpec shard 10 candidate inventory invalid: ${candidateIds.length}`);
    }
    ...
    if (digestStable(entityIds) !== digestStable(expectedEntityIds)) throw new Error(`shard 10 entity inventory mismatch: ${entityIds.join(",")}`);
...
async function fetchBytes(url) {
    const response = await fetch(url, { headers: { "User-Agent": "damageTool-genshin-behavior-shard-10" } });
    if (!response.ok) {
...
function buildSnapshot() {
    ...
    const claim = {
        ...
        shard: {
            ...
            overlapPolicy: "noneWithMaterializedBehaviorShards01To09"
        },
        ...
        travelerVariantPolicy: {
            ...
            note: "No Traveler entity is in shard10; any future Traveler queue entry must remain unresolved until its variant identity is explicit."
        },
        coverage: {
...
function validateBehaviorShard10Snapshot(snapshot, { repositoryRoot: root = repositoryRoot } = {}) {
    const reasons = [];
    ...
    if (shard?.shardId !== shardId || shard?.authoritativeCandidateCount !== queue.candidateCount || digestStable(shard?.authoritativeCandidateIds) !== queue.candidateIdsDigest || digestStable(shard?.authoritativeEntityIds) !== queue.entityIdsDigest || digestStable(shard?.capturedNewEntityIds) !== digestStable(resolvedEntities.map((entity) => entity.entityId)) || Array.isArray(shard?.skippedOverlapEntityIds) === false || shard.skippedOverlapEntityIds.length !== 0 || shard?.overlapPolicy !== "noneWithMaterializedBehaviorShards01To09") reasons.push("authoritativeShardBindingInvalid");
    ...
    if (snapshot?.summary?.shardId !== shardId || snapshot?.summary?.authoritativeCandidates !== 100 || snapshot?.summary?.authoritativeEntities !== 9 || snapshot?.summary?.resolvedEntities !== 9 || snapshot?.summary?.overlapSkippedEntities !== 0 || snapshot?.summary?.ambiguousEntities !== 0 || snapshot?.summary?.resolvedRecords !== 18 || snapshot?.summary?.sourceFamilyCount !== 1 || snapshot?.summary?.certificateEligibleClaims !== 0) reasons.push("summaryInvalid");
...
function validateCheckpoint(checkpoint, { repositoryRoot: root = repositoryRoot } = {}) {
    ...
    if (checkpoint?.claim?.queueShardId !== shardId || checkpoint?.claim?.authoritativeCandidateIdsDigest !== queue.candidateIdsDigest || checkpoint?.claim?.authoritativeEntityIdsDigest !== queue.entityIdsDigest || digestStable(checkpoint?.claim?.requestedEntityIds) !== queue.entityIdsDigest || digestStable(checkpoint?.claim?.capturedEntityIds) !== digestStable(resolvedEntities.map((entity) => entity.entityId)) || Array.isArray(checkpoint?.claim?.skippedOverlapEntityIds) === false || checkpoint.claim.skippedOverlapEntityIds.length !== 0 || checkpoint?.claim?.sourceNamespace !== sourceNamespace || checkpoint?.claim?.canIssueEligibilityCertificate !== false || checkpoint?.claim?.canPromoteCanonical !== false) reasons.push("checkpointClaimInvalid");
    ...
    if (!materializedPath || !fs.existsSync(materializedPath)) reasons.push("materializedSnapshotMissing");
        ...
        if (snapshot && (checkpoint.materializedSnapshot.fieldDigest !== snapshot.fieldDigest || !validateBehaviorShard10Snapshot(snapshot, { repositoryRoot: root }).valid)) reasons.push("materializedSnapshotInvalid");
    }
...
module.exports = {
    ...
    shardId,
    sourceNamespace,
    validateBehaviorShard10Snapshot,
    validateCheckpoint,
    writeSnapshot
};
