...
const {
    ...
    defaultOutputRoot,
    stableJson
} = require("../../scripts/genshinCharacterV2BehaviorBatch12Generate.cjs");
const { auditBehaviorBatch12 } = require("../../scripts/genshinCharacterV2BehaviorBatch12Audit.cjs");
...
test("batch 12 fixes the requested character ID", () => {
    const artifact = loadManifest();
    assert.deepEqual(artifact.batchIds, BATCH_IDS);
    assert.deepEqual(BATCH_IDS, ["10000133"]);
    assert.equal(artifact.summary.batchCharacters, 1);
    assert.equal(artifact.summary.inventoryCharacters, 109);
    assert.equal(artifact.summary.sourceRecords, 15);
    assert.equal(artifact.summary.inventoryCandidates, 14);
    assert.equal(artifact.summary.specs, 14);
    assert.equal(artifact.summary.modifiers, 0);
    assert.equal(artifact.summary.canonical, 0);
    assert.equal(artifact.summary.inventoryInputCandidates, 12);
    assert.equal(artifact.summary.explicitCandidateOperations, 4);
    assert.equal(artifact.summary.unknownCandidateIds, 8);
    assert.equal(artifact.summary.sourceInventoryCandidates, 12);
    assert.equal(artifact.summary.explicitInventoryCandidates, 4);
    assert.equal(artifact.summary.unknownInventoryCandidates, 8);
    assert.deepEqual(artifact.summary.verificationByStatus, { needsReview: 14 });
    assert.deepEqual(artifact.summary.runtimeByStatus, { blocked: 14 });
...
test("raw source pointers and behavior-inventory candidates resolve without inference", () => {
    ...
    assert.equal(explicitCount, 4);
    assert.equal(unknownCount, 8);
    assert.deepEqual(artifact.modifiers, {});
});

test("batch 12 generation, split files, and audit are deterministic", () => {
    ...
    const report = auditBehaviorBatch12({ dataRoot: defaultDataRoot, outputRoot: defaultOutputRoot });
