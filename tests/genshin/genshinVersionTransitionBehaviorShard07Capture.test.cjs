...
const capture = require("../../scripts/genshinVersionTransitionBehaviorShard07Capture.cjs");
const { digestStable } = require("../../scripts/genshinVersionEvidenceValidation.cjs");
const repositoryRoot = path.resolve(__dirname, "../..");

test("behaviorSpec shard 07 is exactly queue-bound and skips shard 06 overlap", () => {
    ...
    assert.deepEqual(snapshot.claim.shard.authoritativeEntityIds, [
        "10000063", "10000064", "10000065", "10000066",
        "10000067", "10000068", "10000069", "10000070"
    ]);
    assert.deepEqual(snapshot.claim.shard.capturedNewEntityIds, capture.resolvedEntities.map((entity) => entity.entityId));
    assert.deepEqual(snapshot.claim.shard.skippedOverlapEntityIds, ["10000063"]);
    assert.equal(snapshot.claim.shard.overlapPolicy, "alreadyCapturedByBehaviorShard06");
    assert.deepEqual(snapshot.claim.resolvedEntities.map((entity) => entity.providerSlug), [
        "yunjin", "kukishinobu", "kamisatoayato", "collei", "dori", "tighnari", "nilou"
    ]);
    ...
    assert.equal(capture.validateBehaviorShard07Snapshot(snapshot, { repositoryRoot }).valid, true);
});

test("shard 07 records immutable raw bindings and embedded provider ids", () => {
    const snapshot = capture.buildSnapshot();
...
test("shard 07 checked-in snapshot and complete checkpoint are deterministic", () => {
    ...
    assert.equal(capture.validateBehaviorShard07Snapshot(checkedIn, { repositoryRoot }).valid, true);
...
test("shard 07 validation rejects forged embedded id, raw digest, comparison, and queue evidence", () => {
    ...
    const embeddedResult = capture.validateBehaviorShard07Snapshot(embedded, { repositoryRoot });
    ...
    const digestResult = capture.validateBehaviorShard07Snapshot(digest, { repositoryRoot });
    ...
    const comparisonResult = capture.validateBehaviorShard07Snapshot(comparison, { repositoryRoot });
    ...
    const queueResult = capture.validateBehaviorShard07Snapshot(queue, { repositoryRoot });
    assert.equal(queueResult.valid, false);
    assert.ok(queueResult.reasons.includes("authoritativeShardBindingInvalid"));
});

test("shard 07 schema declares the fixed queue, embedded id, overlap, and fail-closed gate", () => {
    const schemaPath = path.join(repositoryRoot, "games", "genshin", "data", "schema", "version-transition-behavior-shard-07-snapshot.schema.json");
    const schema = JSON.parse(fs.readFileSync(schemaPath, "utf8"));
    assert.equal(schema.$id, "genshin-version-transition-behavior-shard-07-snapshot.schema.json");
    assert.equal(schema.$defs.claim.properties.shard.properties.shardId.const, capture.shardId);
    assert.deepEqual(schema.$defs.claim.properties.shard.properties.skippedOverlapEntityIds.const, ["10000063"]);
